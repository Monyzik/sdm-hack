...
def make_project_events(
    ...
    p003_release_comm = find_id(communications, project_id="P003", to_team="Release Management")
    p003_regression_milestone = find_id(milestones, project_id="P003", name="Регрессия и аналитика")
    p005_budget = find_id(budgets, project_id="P005")
    p005_backend_allocation = find_id(allocations, project_id="P005", resource_id="R020")

    event_specs = [
        ("P001", "milestone_shifted", "milestone", p001_milestone, "planned_end_date=2026-06-12",
         "planned_end_date=2026-06-26", dt(2026, 6, 9, 16, 20),
         "Security review по скоринговому API сдвинут на 14 дней после повторного запроса модели угроз."),
        ("P001", "task_blocked", "task", p001_security_task, "status=In Progress",
         "status=Blocked; blocker_reason=Security approval pending", dt(2026, 6, 10, 11, 5),
         "Tasktracker получил blocked-задачу по согласованию Security для обработки персональных данных."),
        ("P001", "budget_forecast_changed", "budget", p001_budget,
         "forecast_total_spent=54000000; cost_of_delay_per_day=850000",
         "forecast_total_spent=62000000; cost_of_delay_per_day=1250000", dt(2026, 6, 12, 15, 30),
         "Прогноз бюджета и Cost of Delay выросли после переноса Security review и расширения compliance scope."),
        ("P001", "risk_score_increased", "risk", p001_security_risk, "probability=3; impact=4",
         "probability=5; impact=5", dt(2026, 6, 10, 18, 10),
         "Риск Security стал критическим: расчетный score вырос с 12 до 25 после risk committee."),
        ("P001", "communication_delayed", "communication", p001_security_comm, "status=pending", "status=escalated",
         dt(2026, 6, 12, 10, 45), "Ответ Security по согласованию данных просрочен и эскалирован владельцу проекта."),
        ("P001", "resource_overallocated", "resource_allocation", p001_backend_allocation,
         "planned_hours_per_week=30; total_allocation_percent=120",
         "actual_hours_per_week=35; total_allocation_percent=145", dt(2026, 6, 13, 12, 0),
         "Backend lead оказался распределен между P001 и P002 сверх доступной емкости."),
        ("P002", "milestone_shifted", "milestone",
         find_id(milestones, project_id="P002", name="Интеграция с процессингом"), "planned_end_date=2026-06-05",
         "planned_end_date=2026-06-23", dt(2026, 6, 6, 14, 15),
         "Процессинг перенес окно подключения real-time stream."),
        ("P002", "risk_score_increased", "risk", find_id(risks, project_id="P002", risk_type="Integration"),
         "probability=3; impact=4", "probability=5; impact=4", dt(2026, 6, 7, 9, 40),
         "Расчетный score интеграционного риска вырос с 12 до 20 после отмены первого end-to-end теста."),
        ("P002", "communication_delayed", "communication",
         find_id(communications, project_id="P002", to_team="Card Processing"), "status=pending", "status=escalated",
         dt(2026, 6, 8, 17, 0), "Card Processing не подтвердил новое окно подключения."),
        ("P004", "task_blocked", "task", task_id(tasks_by_project, "P004", 1), "status=In Progress",
         "status=Blocked; blocker_reason=vendor certificates missing", dt(2026, 6, 6, 11, 30),
         "Сертификаты СБП не выданы вендором, тесты gateway заблокированы."),
        ("P004", "budget_forecast_changed", "budget", find_id(budgets, project_id="P004"),
         "forecast_total_spent=39000000", "forecast_total_spent=43000000", dt(2026, 6, 14, 16, 10),
         "Прогноз бюджета gateway вырос из-за дополнительных vendor-сертификатов."),
        ("P004", "resource_overallocated", "resource_allocation",
         find_id(allocations, project_id="P004", resource_id="R004"), "actual_hours_per_week=20",
         "actual_hours_per_week=26; total_allocation_percent=135", dt(2026, 6, 15, 12, 25),
         "Backend-разработчик Core Platform одновременно закрывает P001 и P004."),
        ("P003", "milestone_shifted", "milestone", p003_regression_milestone, "planned_end_date=2026-06-26",
         "planned_end_date=2026-06-30", dt(2026, 6, 18, 10, 20),
         "Регрессионный контур мобильного банка сдвинут после расширения проверок по критическим сценариям."),
        ("P003", "communication_delayed", "communication", p003_release_comm, "status=pending", "status=delayed",
         dt(2026, 6, 18, 16, 5), "Release Management не подтвердил окно pilot release вовремя."),
        ("P005", "budget_forecast_changed", "budget", p005_budget,
         "forecast_total_spent=32000000", "forecast_total_spent=33000000", dt(2026, 6, 17, 13, 20),
         "Forecast по CRM 360 вырос после добавления дополнительных проверок качества данных."),
        ("P005", "resource_overallocated", "resource_allocation", p005_backend_allocation,
         "actual_hours_per_week=30", "actual_hours_per_week=32", dt(2026, 6, 18, 11, 0),
         "Backend CRM получил дополнительную загрузку на финальный пилот."),
    ]
...
def validate_dataset(
    ...
    missing_event_types = required_event_types - {event["event_type"] for event in events}
    ensure(not missing_event_types, f"Missing event types: {sorted(missing_event_types)}")

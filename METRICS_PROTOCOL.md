...
Программный контракт лежит в `sdm/backend/services/metrics.py`:
...
Агентский граф `sdm/agents/project_monitor` также должен использовать этот слой: граф может адаптировать названия полей под промпты и алерты, но не должен повторно реализовывать формулы метрик.
...
Эти метрики больше не являются roadmap: они реализованы в `sdm/backend/services/metrics.py`, возвращаются через `ProjectSummary` / `ProjectMetricsFact` и используются агентским графом `sdm/agents/project_monitor`.

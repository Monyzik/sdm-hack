...
TOP_LEVEL_SOURCE_TOOLS = {
    "get_project_summary": "Сводка проекта",
...
def select_answer_sources(
    ...
    *,
    max_sources: int = MAX_EVIDENCE_SOURCES,
) -> list[dict[str, Any]]:
    targets = list(dict.fromkeys(_normalize_id(value) for value in evidence_ids if value))
    if not targets or max_sources <= 0:
        return []
    # Нормализуем ключи один раз, даже если в ответе много ссылок.
    indexed_records = [
        (
            _normalize_id(record.get("id", "")),
            _normalize_id(record.get("reference") or ""),
            {_normalize_id(value) for value in record.get("_match_keys", [])},
            record,
        )
    ...
    for target in targets:
        # Берём один лучший источник на ссылку, оставляя место остальным ссылкам.
        candidates = []
        for index, (source_id, reference, aliases, record) in enumerate(indexed_records):
            if target == source_id:
                priority = 0
                ...
                continue
            candidates.append((priority, index, source_id, record))
        if not candidates:
            continue
        _, _, source_id, record = min(candidates, key=lambda candidate: candidate[:2])
        ...
        if len(matched) >= max_sources:
            break
    return [
        {key: record[key] for key in PUBLIC_SOURCE_FIELDS if key in record} for record in matched
    ]
...
def _is_source_dict(tool_name: str, value: dict[str, Any], path: tuple[str, ...]) -> bool:
    ...
    if path and path[-1] in STRUCTURAL_SECTIONS:
        return True
    return any(
        field in value and value[field] not in (None, "")
        for field in (*SOURCE_ID_FIELDS, *SOURCE_TITLE_FIELDS)
    )
...
def _source_from_dict(
    ...
    title = (
        TOP_LEVEL_SOURCE_TOOLS.get(tool_name, _title(value, source_type))
        if not path
        else _title(value, source_type)
    )
    ...
    return {
        ...
        "_match_keys": [source_id, *match_keys],
    }
...
def _compact_data(value: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, item in value.items():
        if key.startswith("_") or item is None:
            continue
        # Показываем тот же текст, который читала модель. Его размер уже ограничен
        # при индексации; excerpt нужен только для краткого просмотра.
        result[key] = (
            item if key in SOURCE_TEXT_FIELDS and isinstance(item, str) else _compact_value(item)
        )
    return result

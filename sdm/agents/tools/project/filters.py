...
def _filter_items(
    ...
    *,
    ...
    if query:
        ...
        filtered = [
            ...
            if any(
                ...
                for field in ("id", "external_id", *query_fields)
            )
        ]
    ...
    for field, expected in exact_filters.items():
        ...
        filtered = [
            item for item in filtered if needle == str(item.get(field, "")).strip().casefold()
        ]
    for field, expected in (text_filters or {}).items():
...
def _tool_result(items: list[dict[str, Any]], limit: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "returned_count": min(len(items), limit_value),
        "truncated": len(items) > limit_value,
        "scope": "problem_snapshot",
    }

from __future__ import annotations

import random
from collections import defaultdict
from datetime import date, datetime, time, timedelta
from pathlib import Path
from typing import Any

import pandas as pd

SEED = 42
random.seed(SEED)

DATA_DIR = Path(__file__).resolve().parents[1] / "data" / "demo"
LATEST_SNAPSHOT_DATE = date(2026, 6, 19)
PROJECT_COLUMNS = [
    "id",
    "name",
    "lifecycle_status",
    "priority",
    "start_date",
    "planned_end_date",
    "business_goal",
    "expected_result",
    "business_value",
]

TASK_COLUMNS = [
    "id",
    "project_id",
    "external_id",
    "title",
    "assignee_id",
    "assignee_name",
    "status",
    "priority",
    "planned_due_date",
    "actual_end_date",
    "estimated_hours",
    "spent_hours",
    "is_blocked",
    "blocker_reason",
]

TASK_HISTORY_COLUMNS = [
    "id",
    "project_id",
    "task_id",
    "changed_at",
    "field_changed",
    "old_value",
    "new_value",
    "changed_by",
    "source_system",
]

TASK_COMMENT_COLUMNS = [
    "id",
    "project_id",
    "task_id",
    "author_id",
    "author_name",
    "created_at",
    "channel",
    "text",
    "mentions_count",
    "source_system",
]

MILESTONE_COLUMNS = [
    "id",
    "project_id",
    "name",
    "planned_start_date",
    "planned_end_date",
    "actual_start_date",
    "actual_end_date",
    "status",
    "responsible_team",
]

BUDGET_COLUMNS = [
    "id",
    "project_id",
    "planned_budget",
    "actual_spent",
    "expected_economic_effect",
    "cost_of_delay_per_day",
    "currency",
]

RISK_COLUMNS = [
    "id",
    "project_id",
    "risk_type",
    "description",
    "probability",
    "impact",
    "owner_name",
    "mitigation_plan",
    "status",
    "linked_task_id",
]

COMMUNICATION_COLUMNS = [
    "id",
    "project_id",
    "from_team",
    "to_team",
    "topic",
    "channel",
    "last_message_date",
    "expected_response_date",
    "status",
    "importance",
    "linked_task_id",
]

RESOURCE_COLUMNS = [
    "id",
    "full_name",
    "role",
    "team",
    "available_hours_per_week",
    "hour_rate",
    "seniority",
]

RESOURCE_ALLOCATION_COLUMNS = [
    "id",
    "resource_id",
    "project_id",
    "planned_hours_per_week",
    "actual_hours_per_week",
]

TASK_DEPENDENCY_COLUMNS = [
    "id",
    "project_id",
    "task_id",
    "depends_on_task_id",
    "dependency_type",
    "is_critical_path",
    "lag_days",
    "reason",
]

BUDGET_ITEM_COLUMNS = [
    "id",
    "project_id",
    "budget_id",
    "category",
    "item_name",
    "planned_amount",
    "actual_amount",
    "owner_team",
]

COMMUNICATION_MESSAGE_COLUMNS = [
    "id",
    "project_id",
    "communication_id",
    "message_time",
    "sender_team",
    "recipient_team",
    "channel",
    "message_type",
    "status",
    "summary",
    "linked_task_id",
    "is_escalation",
]

DEPENDENCY_COLUMNS = [
    "id",
    "project_id",
    "dependency_type",
    "depends_on",
    "owner_team",
    "expected_date",
    "status",
    "criticality",
    "linked_task_id",
]

DECISION_COLUMNS = [
    "id",
    "project_id",
    "decision_date",
    "decision_type",
    "description",
    "decision_owner",
    "status",
    "linked_milestone_id",
]

CHANGE_REQUEST_COLUMNS = [
    "id",
    "project_id",
    "request_date",
    "requested_by",
    "change_type",
    "description",
    "requested_budget_delta",
    "requested_timeline_delta_days",
    "status",
]

CSV_COLUMN_LABELS = {
    "id": "идентификатор",
    "project_id": "идентификатор_проекта",
    "name": "название",
    "lifecycle_status": "жизненный_статус",
    "priority": "приоритет",
    "start_date": "дата_старта",
    "planned_end_date": "плановая_дата_окончания",
    "business_goal": "бизнес_цель",
    "expected_result": "ожидаемый_результат",
    "business_value": "бизнес_ценность",
    "external_id": "внешний_идентификатор",
    "title": "заголовок",
    "assignee_id": "идентификатор_исполнителя",
    "assignee_name": "исполнитель",
    "status": "статус",
    "planned_due_date": "плановый_срок",
    "actual_end_date": "фактическая_дата_завершения",
    "estimated_hours": "оценка_часов",
    "spent_hours": "затрачено_часов",
    "is_blocked": "заблокирована",
    "blocker_reason": "причина_блокировки",
    "task_id": "идентификатор_задачи",
    "changed_at": "время_изменения",
    "field_changed": "измененное_поле",
    "old_value": "старое_значение",
    "new_value": "новое_значение",
    "changed_by": "кем_изменено",
    "source_system": "система_источник",
    "author_id": "идентификатор_автора",
    "author_name": "автор",
    "created_at": "время_создания",
    "channel": "канал",
    "text": "текст",
    "mentions_count": "количество_упоминаний",
    "planned_start_date": "плановая_дата_начала",
    "actual_start_date": "фактическая_дата_начала",
    "responsible_team": "ответственная_команда",
    "planned_budget": "плановый_бюджет",
    "actual_spent": "фактические_затраты",
    "expected_economic_effect": "ожидаемый_экономический_эффект",
    "cost_of_delay_per_day": "стоимость_дня_задержки",
    "currency": "валюта",
    "risk_type": "тип_риска",
    "description": "описание",
    "probability": "вероятность",
    "impact": "влияние",
    "owner_name": "ответственный",
    "mitigation_plan": "план_снижения_риска",
    "linked_task_id": "идентификатор_связанной_задачи",
    "from_team": "команда_отправитель",
    "to_team": "команда_получатель",
    "topic": "тема",
    "last_message_date": "дата_последнего_сообщения",
    "expected_response_date": "ожидаемая_дата_ответа",
    "importance": "важность",
    "full_name": "фио",
    "role": "роль",
    "team": "команда",
    "available_hours_per_week": "доступно_часов_в_неделю",
    "hour_rate": "часовая_ставка",
    "seniority": "уровень_специалиста",
    "resource_id": "идентификатор_ресурса",
    "planned_hours_per_week": "план_часов_в_неделю",
    "actual_hours_per_week": "факт_часов_в_неделю",
    "depends_on_task_id": "идентификатор_задачи_предшественника",
    "dependency_type": "тип_зависимости",
    "is_critical_path": "критический_путь",
    "lag_days": "лаг_дней",
    "reason": "причина",
    "budget_id": "идентификатор_бюджета",
    "category": "категория",
    "item_name": "название_статьи",
    "planned_amount": "плановая_сумма",
    "actual_amount": "фактическая_сумма",
    "owner_team": "команда_владелец",
    "communication_id": "идентификатор_коммуникации",
    "message_time": "время_сообщения",
    "sender_team": "команда_автор_сообщения",
    "recipient_team": "команда_адресат_сообщения",
    "message_type": "тип_сообщения",
    "summary": "краткое_содержание",
    "is_escalation": "эскалация",
    "depends_on": "объект_зависимости",
    "expected_date": "ожидаемая_дата",
    "criticality": "критичность",
    "decision_date": "дата_решения",
    "decision_type": "тип_решения",
    "decision_owner": "владелец_решения",
    "linked_milestone_id": "идентификатор_связанной_вехи",
    "request_date": "дата_запроса",
    "requested_by": "инициатор",
    "change_type": "тип_изменения",
    "requested_budget_delta": "запрошенное_изменение_бюджета",
    "requested_timeline_delta_days": "запрошенное_изменение_срока_дней",
}

CSV_BOOLEAN_COLUMNS = {"is_blocked", "is_critical_path", "is_escalation"}
TASK_HISTORY_FIELD_LABELS = {
    "status": "статус",
    "blocker_reason": "причина блокировки",
    "planned_due_date": "плановый срок",
    "estimated_hours": "оценка часов",
    "spent_hours": "затрачено часов",
    "assignee_id": "исполнитель",
}

def iso(value: date | None) -> str:
    return value.isoformat() if value else ""


def iso_dt(value: datetime | None) -> str:
    return value.isoformat(sep=" ", timespec="seconds") if value else ""


def d(year: int, month: int, day: int) -> date:
    return date(year, month, day)


def dt(year: int, month: int, day: int, hour: int, minute: int = 0) -> datetime:
    return datetime.combine(d(year, month, day), time(hour, minute))


class DatasetValidationError(ValueError):
    pass


def ensure(condition: bool, message: str) -> None:
    if not condition:
        raise DatasetValidationError(message)


def make_projects() -> list[dict[str, Any]]:
    return [
        {
            "id": "P001",
            "name": "Скоринговый модуль МСБ",
            "lifecycle_status": "active",
            "priority": "critical",
            "start_date": iso(d(2026, 5, 4)),
            "planned_end_date": iso(d(2026, 6, 30)),
            "business_goal": "Сократить время принятия решения по кредитам МСБ и повысить точность оценки риска.",
            "expected_result": "Модель оценки вероятности дефолта, скоринговый программный интерфейс и интеграция с кредитным конвейером для пилотного сегмента.",
            "business_value": "Ускорение одобрения заявок МСБ до 15 минут и снижение ручной проверки анкет.",
        },
        {
            "id": "P002",
            "name": "Антифрод в реальном времени",
            "lifecycle_status": "active",
            "priority": "critical",
            "start_date": iso(d(2026, 4, 27)),
            "planned_end_date": iso(d(2026, 7, 10)),
            "business_goal": "Снизить потери от мошеннических операций за счет потокового скоринга транзакций.",
            "expected_result": "Конвейер принятия решений в реальном времени с задержкой до 120 мс и витриной расследований.",
            "business_value": "Сокращение антифрод-потерь и снижение доли ручных блокировок операций.",
        },
        {
            "id": "P003",
            "name": "Мобильный банк 2.0",
            "lifecycle_status": "active",
            "priority": "high",
            "start_date": iso(d(2026, 5, 6)),
            "planned_end_date": iso(d(2026, 7, 3)),
            "business_goal": "Обновить ключевые пользовательские сценарии мобильного банка для розничных клиентов.",
            "expected_result": "Новый онбординг, платежи по шаблонам, пуш-подсказки и обновленный профиль клиента.",
            "business_value": "Рост цифровой активности и снижение обращений в контакт-центр по типовым операциям.",
        },
        {
            "id": "P004",
            "name": "Платёжный шлюз",
            "lifecycle_status": "active",
            "priority": "high",
            "start_date": iso(d(2026, 5, 11)),
            "planned_end_date": iso(d(2026, 7, 17)),
            "business_goal": "Объединить карточные и СБП-платежи в едином шлюзе для внутренних продуктов банка.",
            "expected_result": "Высокодоступный шлюз, единый программный интерфейс и контур мониторинга для платежных операций.",
            "business_value": "Сокращение стоимости подключения новых платежных сценариев и повышение уровня сервиса.",
        },
        {
            "id": "P005",
            "name": "CRM 360",
            "lifecycle_status": "active",
            "priority": "medium",
            "start_date": iso(d(2026, 5, 13)),
            "planned_end_date": iso(d(2026, 7, 24)),
            "business_goal": "Собрать единую клиентскую карточку для продаж, сервиса и аналитики.",
            "expected_result": "Золотая запись, витрина клиентских событий и программный интерфейс профиля 360 для клиентских систем.",
            "business_value": "Рост конверсии кросс-продаж и снижение времени поиска клиентской информации.",
        },
    ]


def make_resources() -> list[dict[str, Any]]:
    return [
        {"id": "R001", "full_name": "Алексей Соколов", "role": "Project Manager", "team": "PMO",
         "available_hours_per_week": 40, "hour_rate": 4200, "seniority": "senior"},
        {"id": "R002", "full_name": "Мария Кузнецова", "role": "Business Analyst", "team": "Business Analysis",
         "available_hours_per_week": 40, "hour_rate": 3600, "seniority": "middle+"},
        {"id": "R003", "full_name": "Дмитрий Волков", "role": "Backend Developer", "team": "Core Platform",
         "available_hours_per_week": 40, "hour_rate": 4800, "seniority": "senior"},
        {"id": "R004", "full_name": "Илья Смирнов", "role": "Backend Developer", "team": "Core Platform",
         "available_hours_per_week": 40, "hour_rate": 4300, "seniority": "middle+"},
        {"id": "R005", "full_name": "Ольга Иванова", "role": "Backend Developer", "team": "Payments",
         "available_hours_per_week": 40, "hour_rate": 4100, "seniority": "middle"},
        {"id": "R006", "full_name": "Павел Никитин", "role": "Data Engineer", "team": "Data Platform",
         "available_hours_per_week": 40, "hour_rate": 4500, "seniority": "senior"},
        {"id": "R007", "full_name": "Наталья Федорова", "role": "Data Scientist", "team": "Risk Models",
         "available_hours_per_week": 40, "hour_rate": 4700, "seniority": "senior"},
        {"id": "R008", "full_name": "Сергей Лебедев", "role": "ML Engineer", "team": "Risk Models",
         "available_hours_per_week": 40, "hour_rate": 4600, "seniority": "middle+"},
        {"id": "R009", "full_name": "Анна Попова", "role": "QA Engineer", "team": "Quality",
         "available_hours_per_week": 40, "hour_rate": 3100, "seniority": "middle"},
        {"id": "R010", "full_name": "Роман Егоров", "role": "DevOps Engineer", "team": "Platform",
         "available_hours_per_week": 40, "hour_rate": 4400, "seniority": "senior"},
        {"id": "R011", "full_name": "Виктория Павлова", "role": "Security Engineer", "team": "Security",
         "available_hours_per_week": 32, "hour_rate": 5000, "seniority": "senior"},
        {"id": "R012", "full_name": "Георгий Орлов", "role": "Solution Architect", "team": "Enterprise Architecture",
         "available_hours_per_week": 32, "hour_rate": 5200, "seniority": "principal"},
        {"id": "R013", "full_name": "Екатерина Васильева", "role": "Frontend Developer", "team": "Mobile Web",
         "available_hours_per_week": 40, "hour_rate": 3900, "seniority": "middle+"},
        {"id": "R014", "full_name": "Кирилл Морозов", "role": "Android Developer", "team": "Mobile",
         "available_hours_per_week": 40, "hour_rate": 4200, "seniority": "middle+"},
        {"id": "R015", "full_name": "Дарья Захарова", "role": "iOS Developer", "team": "Mobile",
         "available_hours_per_week": 40, "hour_rate": 4300, "seniority": "middle+"},
        {"id": "R016", "full_name": "Михаил Зайцев", "role": "API Analyst", "team": "Integration",
         "available_hours_per_week": 40, "hour_rate": 3500, "seniority": "middle"},
        {"id": "R017", "full_name": "Татьяна Белова", "role": "QA Automation Engineer", "team": "Quality",
         "available_hours_per_week": 40, "hour_rate": 3600, "seniority": "middle+"},
        {"id": "R018", "full_name": "Артур Комаров", "role": "Product Owner", "team": "Digital Bank",
         "available_hours_per_week": 32, "hour_rate": 4700, "seniority": "senior"},
        {"id": "R019", "full_name": "Софья Андреева", "role": "UX Researcher", "team": "CX",
         "available_hours_per_week": 32, "hour_rate": 3300, "seniority": "middle"},
        {"id": "R020", "full_name": "Никита Новиков", "role": "Backend Developer", "team": "CRM Platform",
         "available_hours_per_week": 40, "hour_rate": 4000, "seniority": "middle"},
        {"id": "R021", "full_name": "Полина Соколова", "role": "Data Analyst", "team": "CRM Analytics",
         "available_hours_per_week": 40, "hour_rate": 3400, "seniority": "middle"},
        {"id": "R022", "full_name": "Евгений Фомин", "role": "Integration Engineer", "team": "Payments",
         "available_hours_per_week": 40, "hour_rate": 4300, "seniority": "senior"},
        {"id": "R023", "full_name": "Лариса Сергеева", "role": "Compliance Officer", "team": "Compliance",
         "available_hours_per_week": 32, "hour_rate": 3900, "seniority": "senior"},
        {"id": "R024", "full_name": "Владислав Крылов", "role": "SRE", "team": "Platform",
         "available_hours_per_week": 40, "hour_rate": 4500, "seniority": "middle+"},
    ]


def make_resource_allocations(resources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    templates = [
        ("R001", "P001", 16, 18),
        ("R001", "P004", 14, 14),
        ("R002", "P001", 24, 26),
        ("R002", "P005", 12, 12),
        ("R003", "P001", 30, 35),
        ("R003", "P002", 18, 23),
        ("R004", "P001", 24, 28),
        ("R004", "P004", 20, 26),
        ("R005", "P004", 28, 30),
        ("R005", "P002", 8, 8),
        ("R006", "P001", 20, 24),
        ("R006", "P005", 14, 18),
        ("R007", "P001", 28, 30),
        ("R007", "P002", 14, 18),
        ("R008", "P002", 30, 34),
        ("R008", "P001", 8, 10),
        ("R009", "P001", 18, 18),
        ("R009", "P003", 12, 12),
        ("R010", "P002", 20, 24),
        ("R010", "P004", 18, 22),
        ("R011", "P001", 12, 18),
        ("R011", "P002", 12, 16),
        ("R011", "P004", 8, 12),
        ("R012", "P001", 10, 12),
        ("R012", "P004", 10, 12),
        ("R013", "P003", 28, 30),
        ("R014", "P003", 28, 30),
        ("R015", "P003", 28, 28),
        ("R016", "P004", 24, 26),
        ("R016", "P005", 8, 8),
        ("R017", "P003", 20, 20),
        ("R017", "P004", 12, 14),
        ("R018", "P003", 18, 18),
        ("R019", "P003", 18, 16),
        ("R020", "P005", 30, 32),
        ("R021", "P005", 28, 30),
        ("R022", "P004", 30, 32),
        ("R023", "P002", 12, 14),
        ("R023", "P005", 10, 10),
        ("R024", "P002", 20, 22),
        ("R024", "P004", 10, 12),
    ]

    rows = []
    for idx, (resource_id, project_id, planned, actual) in enumerate(templates, start=1):
        rows.append(
            {
                "id": f"RA{idx:03d}",
                "resource_id": resource_id,
                "project_id": project_id,
                "planned_hours_per_week": planned,
                "actual_hours_per_week": actual,
            }
        )
    return rows


PROJECT_RESOURCE_IDS = {
    "P001": ["R001", "R002", "R003", "R004", "R006", "R007", "R008", "R009", "R011", "R012"],
    "P002": ["R003", "R005", "R007", "R008", "R010", "R011", "R023", "R024"],
    "P003": ["R009", "R013", "R014", "R015", "R017", "R018", "R019"],
    "P004": ["R001", "R004", "R005", "R010", "R011", "R012", "R016", "R017", "R022", "R024"],
    "P005": ["R002", "R006", "R016", "R020", "R021", "R023"],
}

TASK_CONFIGS = {
    "P001": {"prefix": "МСБ-СКР", "count": 30, "blocked": 5, "overdue_open": 7, "done": 16},
    "P002": {"prefix": "АНФ", "count": 28, "blocked": 4, "overdue_open": 8, "done": 12},
    "P003": {"prefix": "МБ20", "count": 24, "blocked": 1, "overdue_open": 2, "done": 18},
    "P004": {"prefix": "ПШ", "count": 26, "blocked": 2, "overdue_open": 5, "done": 15},
    "P005": {"prefix": "КК360", "count": 22, "blocked": 1, "overdue_open": 2, "done": 16},
}

PROJECT_TASK_OBJECTS = {
    "P001": [
        "согласование безопасности для скорингового программного интерфейса",
        "витрину признаков из АБС",
        "маппинг полей CRM и кредитного конвейера",
        "валидацию модели вероятности дефолта",
        "точку доступа расчета скоринговой карты",
        "логирование решений модели",
        "контроль версий признаков",
        "интеграцию с хранилищем данных",
        "регламент отката модели",
        "сравнительный контур для пилота МСБ",
    ],
    "P002": [
        "конвейер потоковой обработки транзакций",
        "правила блокировки высокорисковых операций",
        "интеграцию с процессингом карт",
        "витрину антифрод-расследований",
        "хранилище признаков реального времени",
        "нагрузочный профиль 120 мс",
        "алерты по всплескам отказов",
        "модель поведенческого скоринга",
        "контур ручного разбора кейсов",
        "отчетность для соответствия требованиям",
    ],
    "P003": [
        "новый онбординг клиента",
        "экран платежей по шаблонам",
        "push-подсказки в профиле",
        "рефакторинг авторизации",
        "аналитику клиентских событий",
        "сравнительный тест главного экрана",
        "дизайн-систему мобильных компонентов",
        "регрессионный пакет автотестов",
        "интеграцию с биометрическим входом",
        "релизные заметки для стора",
    ],
    "P004": [
        "единый платежный программный интерфейс",
        "коннектор СБП",
        "адаптер карточного эквайринга",
        "маршрутизацию платежей",
        "идемпотентность операций",
        "мониторинг уровня сервиса",
        "чек-лист PCI DSS",
        "нагрузочные тесты шлюза",
        "резервный контур обработки",
        "сверку платежных статусов",
    ],
    "P005": [
        "золотую запись клиента",
        "правила дедупликации контактов",
        "витрину клиентских событий",
        "программный интерфейс профиля 360",
        "маппинг каналов продаж",
        "историю взаимодействий",
        "качество данных по ИНН",
        "сегменты кросс-продаж",
        "ролевую модель доступа CRM",
        "дашборд полноты профиля",
    ],
}

TASK_ACTIONS = [
    "Разработать",
    "Настроить",
    "Проверить",
    "Согласовать",
    "Подготовить",
    "Протестировать",
    "Интегрировать",
    "Документировать",
]

BLOCKER_REASONS = {
    "P001": [
        "Ожидается заключение безопасности по доступу к персональным данным",
        "Нет подтверждения уровня сервиса от хранилища данных",
        "Не выделен рецензент бэкенда",
        "Не согласована схема маскирования данных",
        "Зависимость от кредитного конвейера перенесена на следующий спринт",
    ],
    "P002": [
        "Процессинг не подтвердил окно подключения",
        "Безопасность запросила дополнительную модель угроз",
        "Недоступен стенд потоковой обработки",
        "Не согласованы правила разбора ложных срабатываний",
    ],
    "P003": [
        "Ожидается финальное согласование пользовательского опыта по онбордингу",
    ],
    "P004": [
        "Поставщик СБП задерживает тестовые сертификаты",
        "Чек-лист PCI DSS возвращен на доработку",
    ],
    "P005": [
        "Не получено подтверждение владельца данных по CRM-дублям",
    ],
}

BLOCKED_DUE_DATES = {
    "P001": [d(2026, 6, 10), d(2026, 5, 27), d(2026, 6, 4), d(2026, 6, 14), d(2026, 6, 16)],
    "P002": [d(2026, 5, 29), d(2026, 6, 3), d(2026, 6, 9), d(2026, 6, 13)],
    "P003": [d(2026, 6, 16)],
    "P004": [d(2026, 6, 5), d(2026, 6, 12)],
    "P005": [d(2026, 6, 14)],
}


def make_task_title(project_id: str, number: int) -> str:
    if project_id == "P001" and number == 1:
        return "Получить согласование безопасности для скорингового программного интерфейса"
    action = TASK_ACTIONS[(number + random.randint(0, 3)) % len(TASK_ACTIONS)]
    subject = PROJECT_TASK_OBJECTS[project_id][(number - 1) % len(PROJECT_TASK_OBJECTS[project_id])]
    return f"{action} {subject}"


def make_tasks(resources: list[dict[str, Any]]) -> list[dict[str, Any]]:
    resource_by_id = {row["id"]: row for row in resources}
    tasks: list[dict[str, Any]] = []
    next_id = 1

    for project_id, config in TASK_CONFIGS.items():
        resource_ids = PROJECT_RESOURCE_IDS[project_id]
        blocked_count = int(config["blocked"])
        overdue_open_count = int(config["overdue_open"])
        done_count = int(config["done"])
        for number in range(1, config["count"] + 1):
            assignee_id = random.choice(resource_ids)
            assignee = resource_by_id[assignee_id]

            is_blocked = number <= blocked_count
            is_overdue_open = blocked_count < number <= blocked_count + overdue_open_count
            is_done = (
                    blocked_count + overdue_open_count
                    < number
                    <= blocked_count + overdue_open_count + done_count
            )

            if is_blocked:
                status = "Blocked"
                planned_due = BLOCKED_DUE_DATES[project_id][number - 1]
                actual_end = None
                blocker_reason = BLOCKER_REASONS[project_id][(number - 1) % len(BLOCKER_REASONS[project_id])]
                priority = "critical" if number <= 2 else "high"
                spent_hours = random.randint(18, 70)
            elif is_overdue_open:
                status = random.choice(["In Progress", "Review"])
                planned_due = LATEST_SNAPSHOT_DATE - timedelta(days=random.randint(2, 24))
                actual_end = None
                blocker_reason = ""
                priority = random.choice(["high", "high", "medium"])
                spent_hours = random.randint(20, 82)
            elif is_done:
                status = "Done"
                planned_due = d(2026, 5, 12) + timedelta(days=random.randint(0, 31))
                actual_end = planned_due + timedelta(days=random.randint(-2, 6))
                if actual_end > LATEST_SNAPSHOT_DATE:
                    actual_end = LATEST_SNAPSHOT_DATE - timedelta(days=random.randint(0, 2))
                blocker_reason = ""
                priority = random.choice(["medium", "medium", "high", "low"])
                spent_hours = random.randint(12, 74)
            else:
                status = random.choice(["To Do", "In Progress"])
                planned_due = LATEST_SNAPSHOT_DATE + timedelta(days=random.randint(4, 28))
                actual_end = None
                blocker_reason = ""
                priority = random.choice(["medium", "low", "high"])
                spent_hours = random.randint(0, 24)

            estimated_hours = max(8, spent_hours + random.randint(-8, 16))
            task = {
                "id": f"T{next_id:03d}",
                "project_id": project_id,
                "external_id": f"{config['prefix']}-{number:03d}",
                "title": make_task_title(project_id, number),
                "assignee_id": assignee_id,
                "assignee_name": assignee["full_name"],
                "status": status,
                "priority": priority,
                "planned_due_date": iso(planned_due),
                "actual_end_date": iso(actual_end),
                "estimated_hours": estimated_hours,
                "spent_hours": spent_hours,
                "is_blocked": is_blocked,
                "blocker_reason": blocker_reason,
            }
            tasks.append(task)
            next_id += 1

    return tasks


def task_lookup(tasks: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    grouped: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for task in tasks:
        grouped[task["project_id"]].append(task)
    return grouped


def task_id(tasks_by_project: dict[str, list[dict[str, Any]]], project_id: str, number: int) -> str:
    return tasks_by_project[project_id][number - 1]["id"]


def make_milestones() -> list[dict[str, Any]]:
    milestone_templates = {
        "P001": [
            ("Исследование и требования", d(2026, 5, 4), d(2026, 5, 15), d(2026, 5, 4), d(2026, 5, 16), "Done",
             "Business Analysis", 1),
            ("Модель и витрина признаков", d(2026, 5, 18), d(2026, 6, 5), d(2026, 5, 19), d(2026, 6, 8), "Done",
             "Risk Models", 3),
            ("Проверка безопасности и интеграция", d(2026, 6, 8), d(2026, 6, 12), d(2026, 6, 9), None, "Delayed", "Security",
             14),
            ("Пилот в кредитном конвейере", d(2026, 6, 15), d(2026, 6, 30), None, None, "At Risk", "Core Platform", 14),
        ],
        "P002": [
            ("Архитектура потокового контура", d(2026, 4, 27), d(2026, 5, 15), d(2026, 4, 28), d(2026, 5, 17), "Done",
             "Enterprise Architecture", 2),
            ("Интеграция с процессингом", d(2026, 5, 18), d(2026, 6, 5), d(2026, 5, 20), None, "Delayed", "Payments",
             18),
            ("Модель антифрод-решений", d(2026, 5, 25), d(2026, 6, 19), d(2026, 5, 27), None, "At Risk", "Risk Models",
             10),
            ("Приемочное тестирование и промышленный контур", d(2026, 6, 22), d(2026, 7, 10), None, None, "Planned", "Platform", 0),
        ],
        "P003": [
            ("Пользовательский опыт и дизайн-система", d(2026, 5, 6), d(2026, 5, 22), d(2026, 5, 6), d(2026, 5, 21), "Done", "CX", 0),
            ("Разработка клиентских сценариев", d(2026, 5, 25), d(2026, 6, 12), d(2026, 5, 25), d(2026, 6, 12), "Done",
             "Mobile", 0),
            ("Регрессия и аналитика", d(2026, 6, 15), d(2026, 6, 26), d(2026, 6, 15), None, "In Progress", "Quality",
             0),
            ("Пилотный релиз", d(2026, 6, 29), d(2026, 7, 3), None, None, "Planned", "Digital Bank", 0),
        ],
        "P004": [
            ("Контракт программного интерфейса и архитектура", d(2026, 5, 11), d(2026, 5, 29), d(2026, 5, 12), d(2026, 6, 1), "Done",
             "Integration", 3),
            ("СБП и адаптеры эквайринга", d(2026, 6, 1), d(2026, 6, 19), d(2026, 6, 3), None, "At Risk", "Payments", 7),
            ("PCI DSS и безопасность", d(2026, 6, 15), d(2026, 6, 26), d(2026, 6, 17), None, "At Risk", "Security", 5),
            ("Нагрузочное тестирование", d(2026, 6, 29), d(2026, 7, 17), None, None, "Planned", "Platform", 0),
        ],
        "P005": [
            ("Исследование данных", d(2026, 5, 13), d(2026, 5, 29), d(2026, 5, 13), d(2026, 5, 30), "Done", "CRM Analytics",
             1),
            ("Золотая запись", d(2026, 6, 1), d(2026, 6, 19), d(2026, 6, 1), d(2026, 6, 18), "Done", "Data Platform", 0),
            ("Программный интерфейс профиля 360", d(2026, 6, 22), d(2026, 7, 10), None, None, "Planned", "CRM Platform", 0),
            ("Пилот с продажами", d(2026, 7, 13), d(2026, 7, 24), None, None, "Planned", "Sales", 0),
        ],
    }

    rows = []
    idx = 1
    for project_id, milestones in milestone_templates.items():
        for name, planned_start, planned_end, actual_start, actual_end, status, team, _delay_days in milestones:
            rows.append(
                {
                    "id": f"M{idx:03d}",
                    "project_id": project_id,
                    "name": name,
                    "planned_start_date": iso(planned_start),
                    "planned_end_date": iso(planned_end),
                    "actual_start_date": iso(actual_start),
                    "actual_end_date": iso(actual_end),
                    "status": status,
                    "responsible_team": team,
                }
            )
            idx += 1
    return rows


def make_budgets() -> list[dict[str, Any]]:
    return [
        {"id": "B001", "project_id": "P001", "planned_budget": 48_000_000, "actual_spent": 41_500_000,
         "expected_economic_effect": 57_000_000, "cost_of_delay_per_day": 1_250_000,
         "currency": "RUB"},
        {"id": "B002", "project_id": "P002", "planned_budget": 72_000_000, "actual_spent": 64_000_000,
         "expected_economic_effect": 112_000_000,
         "cost_of_delay_per_day": 1_800_000, "currency": "RUB"},
        {"id": "B003", "project_id": "P003", "planned_budget": 42_000_000, "actual_spent": 29_500_000,
         "expected_economic_effect": 76_000_000, "cost_of_delay_per_day": 650_000,
         "currency": "RUB"},
        {"id": "B004", "project_id": "P004", "planned_budget": 38_000_000, "actual_spent": 31_000_000,
         "expected_economic_effect": 59_000_000, "cost_of_delay_per_day": 950_000,
         "currency": "RUB"},
        {"id": "B005", "project_id": "P005", "planned_budget": 34_000_000, "actual_spent": 22_500_000,
         "expected_economic_effect": 68_000_000, "cost_of_delay_per_day": 520_000,
         "currency": "RUB"},
    ]


def make_risks(tasks_by_project: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    risk_specs = [
        ("P001", "Security", "Безопасность может не согласовать обработку персональных данных для скорингового программного интерфейса.", 5, 5,
         "Виктория Павлова", "Вынести модель угроз на комитет по рискам и подготовить маскирование полей.", "escalated",
         task_id(tasks_by_project, "P001", 1), d(2026, 6, 10)),
        ("P001", "Resource",
         "Ведущий бэкенд-разработчик распределен между двумя критическими проектами и работает выше доступной емкости.", 4, 5,
         "Елена Морозова", "Перенести часть задач на второго бэкенд-разработчика и зафиксировать лимит незавершенной работы.", "active",
         task_id(tasks_by_project, "P001", 6), d(2026, 5, 29)),
        ("P001", "Budget", "Прогноз бюджета превышает план из-за доработок безопасности и интеграций.", 4, 4,
         "Елена Морозова", "Согласовать сокращение объема и отдельный резерв на доработки соответствия требованиям.", "active",
         task_id(tasks_by_project, "P001", 9), d(2026, 6, 12)),
        ("P001", "Dependency", "Хранилище данных не подтвердило стабильный уровень сервиса для витрины признаков МСБ.", 3, 5, "Павел Никитин",
         "Согласовать резервную выгрузку и ежедневный контроль качества данных.", "mitigating",
         task_id(tasks_by_project, "P001", 2), d(2026, 6, 5)),
        ("P001", "Data Quality", "Исторические признаки по части заявок МСБ неполные.", 3, 4, "Наталья Федорова",
         "Запустить сверку с кредитным конвейером и исключить неполные выборки.", "active",
         task_id(tasks_by_project, "P001", 4), d(2026, 5, 29)),
        ("P002", "Performance", "Задержка принятия решений в реальном времени превышает целевой лимит 120 мс при пиковой нагрузке.", 4, 5,
         "Сергей Лебедев", "Оптимизировать поиск признаков и включить предохранитель отказов.", "escalated",
         task_id(tasks_by_project, "P002", 6), d(2026, 5, 29)),
        ("P002", "Integration", "Процессинг карт переносит окно подключения и блокирует сквозные тесты.", 5, 4,
         "Евгений Фомин", "Эскалировать слот подключения и подготовить контур-заглушку.", "escalated",
         task_id(tasks_by_project, "P002", 1), d(2026, 6, 3)),
        ("P002", "Compliance", "Нужно доказать контролируемый уровень ложных срабатываний для операций клиентов.", 4, 4,
         "Лариса Сергеева", "Подготовить отчетность по тестовой выборке и критерии ручного разбора.", "active",
         task_id(tasks_by_project, "P002", 8), d(2026, 6, 5)),
        ("P002", "Security", "Дополнительная модель угроз задерживает доступ к продуктивным событиям.", 3, 5,
         "Виктория Павлова", "Провести рабочую встречу по модели угроз и согласовать доступ только на чтение.", "active",
         task_id(tasks_by_project, "P002", 2), d(2026, 6, 9)),
        ("P002", "Resource", "Инженер машинного обучения перегружен задачами расследований и оптимизации производительности.", 3, 4,
         "Андрей Романов", "Выделить отдельный слот на оптимизацию модели.", "mitigating",
         task_id(tasks_by_project, "P002", 7), d(2026, 5, 29)),
        ("P003", "Release", "Проверка в магазине приложений может занять больше стандартного окна.", 2, 3, "Артур Комаров",
         "Отправить сборку заранее и подготовить поэтапный запуск.", "active", task_id(tasks_by_project, "P003", 10),
         d(2026, 6, 12)),
        ("P003", "UX", "Пользователи могут хуже проходить обновленный онбординг без подсказок.", 2, 3,
         "Софья Андреева", "Провести дополнительное исследование удобства на контрольной группе.", "mitigating",
         task_id(tasks_by_project, "P003", 1), d(2026, 5, 29)),
        ("P003", "Dependency", "Биометрический вход зависит от смежного релиза авторизации.", 2, 4, "Екатерина Васильева",
         "Оставить переключатель функции и резервный сценарий входа по паролю.", "active", task_id(tasks_by_project, "P003", 9),
         d(2026, 6, 5)),
        ("P004", "Vendor", "Поставщик СБП задерживает тестовые сертификаты для шлюза.", 4, 4, "Сергей Ковалев",
         "Эскалировать через менеджера поставщика и подготовить тестовый имитатор.", "active", task_id(tasks_by_project, "P004", 1),
         d(2026, 6, 5)),
        ("P004", "Security", "Чек-лист PCI DSS требует дополнительного аудита хранения токенов.", 3, 5,
         "Виктория Павлова", "Разделить объем аудита и вынести спорные пункты на комитет изменений.", "active",
         task_id(tasks_by_project, "P004", 2), d(2026, 6, 12)),
        ("P004", "Performance", "Шлюз может не выдержать пиковые платежные окна без настройки командой надежности.", 3, 4,
         "Владислав Крылов", "Добавить синтетическую нагрузку и автоалерты по задержке.", "mitigating",
         task_id(tasks_by_project, "P004", 8), d(2026, 6, 12)),
        ("P004", "Budget", "Дополнительные сертификаты поставщика увеличивают ожидаемые затраты проекта.", 3, 3,
         "Сергей Ковалев", "Зафиксировать лимит закупки и убрать часть некритичных работ.", "active",
         task_id(tasks_by_project, "P004", 3), d(2026, 6, 12)),
        ("P005", "Data Quality", "Дубликаты клиентов в CRM и хранилище данных расходятся по ключевым атрибутам.", 3, 4,
         "Полина Соколова", "Ввести оценку уверенности для золотой записи и ручную валидацию топ-сегментов.", "active",
         task_id(tasks_by_project, "P005", 7), d(2026, 5, 29)),
        ("P005", "Adoption", "Команды продаж могут продолжить использовать старые карточки клиентов.", 2, 3,
         "Ольга Беляева", "Провести пилот с двумя регионами и собрать обратную связь.", "mitigating",
         task_id(tasks_by_project, "P005", 5), d(2026, 6, 5)),
        ("P005", "Compliance", "Нужна проверка ролей доступа к чувствительным клиентским атрибутам.", 2, 4,
         "Лариса Сергеева", "Согласовать матрицу ролей до начала пилота.", "active",
         task_id(tasks_by_project, "P005", 9), d(2026, 6, 12)),
    ]

    rows = []
    for idx, spec in enumerate(risk_specs, start=1):
        (
            project_id,
            risk_type,
            description,
            probability,
            impact,
            owner,
            mitigation,
            status,
            linked_task_id,
            _active_from,
        ) = spec
        rows.append(
            {
                "id": f"RK{idx:03d}",
                "project_id": project_id,
                "risk_type": risk_type,
                "description": description,
                "probability": probability,
                "impact": impact,
                "owner_name": owner,
                "mitigation_plan": mitigation,
                "status": status,
                "linked_task_id": linked_task_id,
            }
        )
    return rows


def make_communications(tasks_by_project: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    def row(
            idx: int,
            project_id: str,
            from_team: str,
            to_team: str,
            topic: str,
            channel: str,
            last_message_date: date,
            expected_response_date: date,
            status: str,
            importance: str,
            linked_task_id: str,
    ) -> dict[str, Any]:
        return {
            "id": f"C{idx:03d}",
            "project_id": project_id,
            "from_team": from_team,
            "to_team": to_team,
            "topic": topic,
            "channel": channel,
            "last_message_date": iso(last_message_date),
            "expected_response_date": iso(expected_response_date),
            "status": status,
            "importance": importance,
            "linked_task_id": linked_task_id,
        }

    specs = [
        ("P001", "Risk Models", "Security", "Согласование обработки ПДн для скорингового программного интерфейса", "Teams", d(2026, 6, 4),
         d(2026, 6, 5), "escalated", "critical", task_id(tasks_by_project, "P001", 1)),
        ("P001", "Data Platform", "DWH", "Уровень сервиса витрины признаков МСБ", "Email", d(2026, 6, 6), d(2026, 6, 7), "delayed",
         "high", task_id(tasks_by_project, "P001", 2)),
        ("P001", "Core Platform", "Credit Conveyor", "Окно интеграционного тестирования скоринга", "Jira",
         d(2026, 6, 12), d(2026, 6, 14), "delayed", "high", task_id(tasks_by_project, "P001", 5)),
        ("P001", "PMO", "Business Owner", "Решение по сокращению объема после роста бюджета", "Email", d(2026, 6, 17),
         d(2026, 6, 18), "pending", "high", task_id(tasks_by_project, "P001", 9)),
        ("P001", "Risk Models", "Data Quality", "Сверка неполных признаков по заявкам МСБ", "Confluence",
         d(2026, 6, 13), d(2026, 6, 17), "responded", "medium", task_id(tasks_by_project, "P001", 4)),
        ("P002", "Fraud Platform", "Card Processing", "Окно подключения потока транзакций", "Email", d(2026, 6, 1),
         d(2026, 6, 3), "escalated", "critical", task_id(tasks_by_project, "P002", 1)),
        ("P002", "Risk Models", "Compliance", "Критерии проверки ложных срабатываний", "Teams", d(2026, 6, 7), d(2026, 6, 10),
         "delayed", "high", task_id(tasks_by_project, "P002", 8)),
        ("P002", "Platform", "SRE", "Профиль процессора при поиске признаков", "Jira", d(2026, 6, 13), d(2026, 6, 14), "responded",
         "medium", task_id(tasks_by_project, "P002", 6)),
        ("P002", "Security", "Fraud Platform", "Модель угроз для событий реального времени", "Confluence", d(2026, 6, 11),
         d(2026, 6, 13), "pending", "high", task_id(tasks_by_project, "P002", 2)),
        ("P002", "PMO", "Business Owner", "Решение по запуску пилота без части правил", "Email", d(2026, 6, 16),
         d(2026, 6, 18), "pending", "high", task_id(tasks_by_project, "P002", 3)),
        ("P003", "Mobile", "CX", "Финальное согласование онбординга", "Teams", d(2026, 6, 15), d(2026, 6, 16), "responded",
         "medium", task_id(tasks_by_project, "P003", 1)),
        ("P003", "Mobile", "Analytics", "Событийная схема для нового профиля", "Jira", d(2026, 6, 14), d(2026, 6, 17),
         "responded", "medium", task_id(tasks_by_project, "P003", 5)),
        ("P003", "Digital Bank", "Release Management", "Слот пилотного релиза", "Email", d(2026, 6, 18), d(2026, 6, 19),
         "pending", "medium", task_id(tasks_by_project, "P003", 10)),
        ("P003", "Quality", "Mobile", "Регрессионный прогон критических сценариев", "Jira", d(2026, 6, 17), d(2026, 6, 18),
         "responded", "medium", task_id(tasks_by_project, "P003", 8)),
        ("P004", "Payments", "SBP Vendor", "Тестовые сертификаты СБП", "Email", d(2026, 6, 6), d(2026, 6, 8), "delayed",
         "critical", task_id(tasks_by_project, "P004", 1)),
        ("P004", "Security", "Payments", "Проверка хранения токенов PCI DSS", "Confluence", d(2026, 6, 11), d(2026, 6, 13),
         "delayed", "high", task_id(tasks_by_project, "P004", 2)),
        ("P004", "Platform", "SRE", "Нагрузочное окно шлюза", "Teams", d(2026, 6, 16), d(2026, 6, 17), "responded",
         "medium", task_id(tasks_by_project, "P004", 8)),
        ("P004", "PMO", "Business Owner", "Перенос части сценариев торговых точек", "Email", d(2026, 6, 18), d(2026, 6, 19),
         "pending", "medium", task_id(tasks_by_project, "P004", 3)),
        ("P005", "CRM Analytics", "Data Owners", "Правила дедупликации контактов", "Teams", d(2026, 6, 12),
         d(2026, 6, 14), "responded", "medium", task_id(tasks_by_project, "P005", 2)),
        ("P005", "CRM Platform", "Compliance", "Матрица доступа к профилю 360", "Confluence", d(2026, 6, 13),
         d(2026, 6, 17), "pending", "medium", task_id(tasks_by_project, "P005", 9)),
        ("P005", "Sales", "CRM Platform", "Сценарии пилота для регионов", "Email", d(2026, 6, 18), d(2026, 6, 19),
         "pending", "low", task_id(tasks_by_project, "P005", 5)),
        ("P005", "Data Platform", "CRM Analytics", "Сверка золотой записи после дедупликации", "Jira", d(2026, 6, 16),
         d(2026, 6, 18), "responded", "medium", task_id(tasks_by_project, "P005", 1)),
    ]

    return [row(idx, *spec) for idx, spec in enumerate(specs, start=1)]


def parse_date(value: str | None) -> date | None:
    if not value:
        return None
    return date.fromisoformat(value)


def find_id(rows: list[dict[str, Any]], **conditions: Any) -> str:
    for row in rows:
        if all(row.get(key) == value for key, value in conditions.items()):
            return row["id"]
    raise ValueError(f"Row not found: {conditions}")


def make_task_dependencies(tasks_by_project: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    specs = [
        ("P001", 5, 1, "blocks", True, 1, "Согласование безопасности требуется до интеграции и пилотных работ."),
        ("P001", 6, 2, "blocks", True, 0, "Подтверждение уровня сервиса хранилища данных требуется до валидации признаков."),
        ("P001", 9, 4, "requires", False, 2, "Процедура отката зависит от проверенной документации модели."),
        ("P002", 4, 1, "blocks", True, 1, "Интеграционное тестирование ожидает окно процессинга."),
        ("P002", 5, 2, "blocks", True, 1, "Настройка хранилища признаков зависит от согласования модели угроз."),
        ("P002", 6, 4, "blocks", True, 0, "Оптимизация задержки зависит от стабильного адаптера процессинга."),
        ("P003", 6, 1, "related", False, 1, "Онбординг должен быть согласован с пользовательским опытом."),
        ("P003", 8, 6, "blocks", False, 0, "Регрессионный пакет зависит от стабилизированного клиентского сценария."),
        ("P004", 4, 1, "blocks", True, 1, "Маршрутизация платежей зависит от поставки сертификатов поставщика."),
        ("P004", 8, 2, "blocks", True, 2, "Нагрузочное тестирование зависит от завершенного чек-листа PCI DSS."),
        ("P005", 4, 1, "blocks", True, 1, "Маппинг золотой записи зависит от правил дедупликации."),
        ("P005", 8, 4, "related", False, 0, "Сегменты кросс-продаж зависят от стабильного качества профиля."),
    ]

    rows = []
    for idx, (project_id, task_number, depends_on_number, dependency_type, is_critical, lag_days, reason) in enumerate(
            specs, start=1):
        rows.append(
            {
                "id": f"TD{idx:03d}",
                "project_id": project_id,
                "task_id": task_id(tasks_by_project, project_id, task_number),
                "depends_on_task_id": task_id(tasks_by_project, project_id, depends_on_number),
                "dependency_type": dependency_type,
                "is_critical_path": is_critical,
                "lag_days": lag_days,
                "reason": reason,
            }
        )
    return rows


def make_budget_line_items(budgets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    budget_id = {row["project_id"]: row["id"] for row in budgets}
    specs = [
        ("P001", "Engineering", "Model development", "Risk Models", 10_000_000, 8_500_000),
        ("P001", "Integration", "Core Platform integration", "Core Platform", 14_000_000, 12_000_000),
        ("P001", "Compliance", "Security and approvals", "Security", 16_000_000, 13_000_000),
        ("P001", "Testing", "Pilot rollout", "PMO", 8_000_000, 8_000_000),
        ("P002", "Engineering", "Streaming core", "Fraud Platform", 20_000_000, 18_000_000),
        ("P002", "Integration", "Card processing adapter", "Payments", 18_000_000, 16_000_000),
        ("P002", "Modeling", "Feature store and scoring", "Risk Models", 20_000_000, 14_000_000),
        ("P002", "Compliance", "UAT and sign-off", "Compliance", 14_000_000, 16_000_000),
        ("P003", "UX", "Mobile UX and design", "Mobile", 10_000_000, 8_000_000),
        ("P003", "Product", "Client scenarios", "Mobile", 10_000_000, 7_000_000),
        ("P003", "Analytics", "Event tracking", "CRM Analytics", 12_000_000, 9_000_000),
        ("P003", "Release", "Stabilization", "Quality", 10_000_000, 5_500_000),
        ("P004", "Engineering", "Gateway core", "Payments", 10_000_000, 9_000_000),
        ("P004", "Integration", "SBP and acquiring adapters", "Payments", 12_000_000, 8_000_000),
        ("P004", "Security", "PCI DSS workstream", "Security", 8_000_000, 7_000_000),
        ("P004", "Observability", "Load testing and monitoring", "Platform", 8_000_000, 7_000_000),
        ("P005", "Data Quality", "Golden record and dedupe", "CRM Analytics", 9_000_000, 6_000_000),
        ("P005", "Engineering", "CRM profile API", "CRM Platform", 9_000_000, 6_000_000),
        ("P005", "Governance", "Data quality controls", "Data Platform", 8_000_000, 5_000_000),
        ("P005", "Rollout", "Pilot rollout", "Sales", 8_000_000, 5_500_000),
    ]

    rows = []
    for idx, (project_id, category, item_name, owner_team, planned_amount, actual_amount) in enumerate(specs, start=1):
        rows.append(
            {
                "id": f"BI{idx:03d}",
                "project_id": project_id,
                "budget_id": budget_id[project_id],
                "category": category,
                "item_name": item_name,
                "planned_amount": planned_amount,
                "actual_amount": actual_amount,
                "owner_team": owner_team,
            }
        )
    return rows


def make_communication_messages(communications: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = []
    next_id = 1
    for communication in communications:
        project_id = communication["project_id"]
        comm_id = communication["id"]
        topic = communication["topic"]
        channel = communication["channel"]
        from_team = communication["from_team"]
        to_team = communication["to_team"]
        last_date = parse_date(communication["last_message_date"])
        response_date = parse_date(communication["expected_response_date"])
        linked_task_id = communication["linked_task_id"]
        ensure(last_date is not None, f"Communication {comm_id} has no last_message_date")
        ensure(response_date is not None, f"Communication {comm_id} has no expected_response_date")

        rows.append(
            {
                "id": f"CM{next_id:03d}",
                "project_id": project_id,
                "communication_id": comm_id,
                "message_time": iso_dt(dt(last_date.year, last_date.month, last_date.day, 9, 15)),
                "sender_team": from_team,
                "recipient_team": to_team,
                "channel": channel,
                "message_type": "request",
                "status": "sent",
                "summary": f"Первичный запрос по теме: {topic}",
                "linked_task_id": linked_task_id,
                "is_escalation": False,
            }
        )
        next_id += 1

        if communication["status"] == "responded":
            sender_team, recipient_team, message_type, status, summary, escalation = (
                to_team,
                from_team,
                "response",
                "replied",
                f"Ответ по теме: {topic}",
                False,
            )
        elif communication["status"] == "escalated":
            sender_team, recipient_team, message_type, status, summary, escalation = (
                "PMO",
                to_team,
                "escalation",
                "escalated",
                f"Эскалация по теме: {topic}",
                True,
            )
        elif communication["status"] == "delayed":
            sender_team, recipient_team, message_type, status, summary, escalation = (
                from_team,
                to_team,
                "reminder",
                "waiting",
                f"Напоминание по теме: {topic}",
                False,
            )
        else:
            sender_team, recipient_team, message_type, status, summary, escalation = (
                from_team,
                to_team,
                "follow_up",
                "waiting",
                f"Фоллоу-ап по теме: {topic}",
                False,
            )

        rows.append(
            {
                "id": f"CM{next_id:03d}",
                "project_id": project_id,
                "communication_id": comm_id,
                "message_time": iso_dt(dt(response_date.year, response_date.month, response_date.day, 15, 30)),
                "sender_team": sender_team,
                "recipient_team": recipient_team,
                "channel": channel,
                "message_type": message_type,
                "status": status,
                "summary": summary,
                "linked_task_id": linked_task_id,
                "is_escalation": escalation,
            }
        )
        next_id += 1

    return rows


def make_task_history(tasks_by_project: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    specs = [
        ("P001", 1, dt(2026, 6, 10, 11, 5), "status", "In Progress", "Blocked", "R011"),
        ("P001", 1, dt(2026, 6, 10, 11, 6), "blocker_reason", "", "Security approval pending", "R011"),
        ("P001", 2, dt(2026, 6, 7, 10, 20), "planned_due_date", "2026-06-03", "2026-06-07", "R006"),
        ("P001", 5, dt(2026, 6, 12, 15, 40), "status", "To Do", "Blocked", "R003"),
        ("P001", 6, dt(2026, 6, 13, 12, 15), "assignee_id", "R004", "R003", "R001"),
        ("P002", 1, dt(2026, 6, 3, 14, 10), "status", "In Progress", "Blocked", "R022"),
        ("P002", 4, dt(2026, 6, 6, 16, 30), "planned_due_date", "2026-06-05", "2026-06-23", "R001"),
        ("P002", 6, dt(2026, 6, 12, 18, 0), "spent_hours", "46", "72", "R010"),
        ("P003", 8, dt(2026, 6, 18, 10, 20), "planned_due_date", "2026-06-26", "2026-06-30", "R017"),
        ("P003", 10, dt(2026, 6, 18, 16, 5), "status", "To Do", "In Progress", "R018"),
        ("P004", 1, dt(2026, 6, 6, 11, 30), "status", "In Progress", "Blocked", "R022"),
        ("P004", 2, dt(2026, 6, 12, 13, 10), "status", "Review", "Blocked", "R011"),
        ("P004", 8, dt(2026, 6, 15, 12, 25), "assignee_id", "R024", "R004", "R001"),
        ("P005", 2, dt(2026, 6, 14, 9, 50), "status", "In Progress", "Review", "R021"),
        ("P005", 7, dt(2026, 6, 17, 13, 20), "estimated_hours", "32", "48", "R021"),
        ("P005", 9, dt(2026, 6, 18, 11, 0), "status", "To Do", "In Progress", "R023"),
    ]

    rows = []
    for idx, (project_id, task_number, changed_at, field, old_value, new_value, changed_by) in enumerate(specs, start=1):
        rows.append(
            {
                "id": f"TH{idx:03d}",
                "project_id": project_id,
                "task_id": task_id(tasks_by_project, project_id, task_number),
                "changed_at": iso_dt(changed_at),
                "field_changed": field,
                "old_value": old_value,
                "new_value": new_value,
                "changed_by": changed_by,
                "source_system": "tasktracker",
            }
        )
    return rows


def make_task_comments(
    tasks_by_project: dict[str, list[dict[str, Any]]],
    resources: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    resource_name = {resource["id"]: resource["full_name"] for resource in resources}
    specs = [
        ("P001", 1, "R007", dt(2026, 6, 4, 10, 30), "Teams", "@Безопасность нужна дата согласования модели угроз по скоринговому программному интерфейсу.", 1),
        ("P001", 1, "R011", dt(2026, 6, 5, 17, 10), "Teams", "Нужны дополнительные материалы по маскированию ПДн, без них согласование не дадим.", 0),
        ("P001", 1, "R001", dt(2026, 6, 10, 11, 10), "Jira", "Блокер зафиксирован, выношу вопрос на управляющий комитет.", 0),
        ("P001", 2, "R006", dt(2026, 6, 7, 12, 0), "Email", "Хранилище данных пока не подтверждает уровень сервиса на витрину признаков, нужна резервная выгрузка.", 0),
        ("P001", 9, "R001", dt(2026, 6, 12, 16, 0), "Jira", "Нужен выбор: сокращение объема или резерв на доработки по соответствию требованиям?", 0),
        ("P002", 1, "R022", dt(2026, 6, 3, 15, 20), "Email", "Окно процессинга перенесено, сквозные тесты не стартуют.", 0),
        ("P002", 6, "R010", dt(2026, 6, 12, 18, 20), "Jira", "Задержка выше целевого лимита, поиск признаков требует оптимизации.", 0),
        ("P002", 8, "R023", dt(2026, 6, 10, 14, 15), "Teams", "Критерии ложных срабатываний требуют отдельного отчета для подтверждения соответствия требованиям.", 0),
        ("P003", 8, "R017", dt(2026, 6, 18, 10, 35), "Jira", "Регрессия расширена на критические платежные сценарии, релизное окно под риском.", 0),
        ("P003", 10, "R018", dt(2026, 6, 18, 16, 10), "Email", "Ждем подтверждение релизного окна, вопрос открыт до конца дня.", 0),
        ("P004", 1, "R022", dt(2026, 6, 6, 12, 0), "Email", "Поставщик не выдал тестовые сертификаты СБП, тесты шлюза заблокированы.", 0),
        ("P004", 2, "R011", dt(2026, 6, 12, 13, 30), "Confluence", "Проверочный список PCI DSS возвращен на доработку по хранению токенов.", 0),
        ("P004", 8, "R024", dt(2026, 6, 15, 12, 40), "Jira", "Для нагрузочного окна нужна дополнительная поддержка бэкенд-команды.", 0),
        ("P005", 2, "R021", dt(2026, 6, 14, 10, 5), "Teams", "Дедупликация контактов дала спорные пары, нужна ручная проверка топ-сегментов.", 0),
        ("P005", 7, "R021", dt(2026, 6, 17, 13, 30), "Jira", "Оценка выросла из-за дополнительного контроля качества данных.", 0),
        ("P005", 9, "R023", dt(2026, 6, 18, 11, 20), "Confluence", "Матрица доступа готовится к проверке соответствия требованиям.", 0),
    ]

    rows = []
    for idx, (project_id, task_number, author_id, created_at, channel, text, mentions_count) in enumerate(specs, start=1):
        rows.append(
            {
                "id": f"TC{idx:03d}",
                "project_id": project_id,
                "task_id": task_id(tasks_by_project, project_id, task_number),
                "author_id": author_id,
                "author_name": resource_name[author_id],
                "created_at": iso_dt(created_at),
                "channel": channel,
                "text": text,
                "mentions_count": mentions_count,
                "source_system": "tasktracker",
            }
        )
    return rows


def make_dependencies(tasks_by_project: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    specs = [
        (
            "P001",
            "internal_system",
            "DWH feature mart",
            "Data Platform",
            d(2026, 6, 7),
            "delayed",
            "high",
            task_id(tasks_by_project, "P001", 2),
        ),
        (
            "P001",
            "approval",
            "Security approval",
            "Security",
            d(2026, 6, 5),
            "blocked",
            "critical",
            task_id(tasks_by_project, "P001", 1),
        ),
        (
            "P001",
            "management_decision",
            "Scope cut decision",
            "PMO",
            d(2026, 6, 18),
            "pending",
            "high",
            task_id(tasks_by_project, "P001", 9),
        ),
        (
            "P002",
            "internal_system",
            "Card processing stream",
            "Card Processing",
            d(2026, 6, 3),
            "blocked",
            "critical",
            task_id(tasks_by_project, "P002", 1),
        ),
        (
            "P002",
            "approval",
            "False positive criteria",
            "Compliance",
            d(2026, 6, 10),
            "delayed",
            "high",
            task_id(tasks_by_project, "P002", 8),
        ),
        (
            "P003",
            "release_window",
            "Pilot release slot",
            "Release Management",
            d(2026, 6, 19),
            "pending",
            "medium",
            task_id(tasks_by_project, "P003", 10),
        ),
        (
            "P004",
            "vendor",
            "SBP test certificates",
            "SBP Vendor",
            d(2026, 6, 8),
            "delayed",
            "critical",
            task_id(tasks_by_project, "P004", 1),
        ),
        (
            "P004",
            "approval",
            "PCI DSS token storage review",
            "Security",
            d(2026, 6, 13),
            "delayed",
            "high",
            task_id(tasks_by_project, "P004", 2),
        ),
        (
            "P005",
            "data_owner",
            "CRM duplicate ownership",
            "Data Owners",
            d(2026, 6, 14),
            "responded",
            "medium",
            task_id(tasks_by_project, "P005", 2),
        ),
        (
            "P005",
            "approval",
            "Profile 360 access matrix",
            "Compliance",
            d(2026, 6, 17),
            "pending",
            "medium",
            task_id(tasks_by_project, "P005", 9),
        ),
    ]

    rows = []
    for idx, (project_id, dependency_type, depends_on, owner_team, expected_date, status, criticality, linked_task_id) in enumerate(
        specs,
        start=1,
    ):
        rows.append(
            {
                "id": f"D{idx:03d}",
                "project_id": project_id,
                "dependency_type": dependency_type,
                "depends_on": depends_on,
                "owner_team": owner_team,
                "expected_date": iso(expected_date),
                "status": status,
                "criticality": criticality,
                "linked_task_id": linked_task_id,
            }
        )
    return rows


def make_decisions(milestones: list[dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        (
            "P001",
            d(2026, 6, 12),
            "scope_change",
            "Перенести часть интеграции на период после пилота, чтобы разблокировать проверку безопасности.",
            "Елена Морозова",
            "pending",
            find_id(milestones, project_id="P001", name="Проверка безопасности и интеграция"),
        ),
        (
            "P001",
            d(2026, 6, 13),
            "budget_reserve",
            "Согласовать дополнительный резерв на доработки по соответствию требованиям или сократить объем пилота.",
            "Управляющий комитет",
            "pending",
            find_id(milestones, project_id="P001", name="Пилот в кредитном конвейере"),
        ),
        (
            "P002",
            d(2026, 6, 7),
            "pilot_scope",
            "Запускать пилот антифрод-решений без части правил до завершения интеграции с процессингом.",
            "Андрей Романов",
            "pending",
            find_id(milestones, project_id="P002", name="Интеграция с процессингом"),
        ),
        (
            "P003",
            d(2026, 6, 18),
            "release_go",
            "Подтвердить окно пилотного релиза после расширенного регрессионного прогона.",
            "Артур Комаров",
            "approved",
            find_id(milestones, project_id="P003", name="Пилотный релиз"),
        ),
        (
            "P004",
            d(2026, 6, 14),
            "vendor_escalation",
            "Эскалировать задержку сертификатов СБП через менеджера поставщика.",
            "Сергей Ковалев",
            "approved",
            find_id(milestones, project_id="P004", name="СБП и адаптеры эквайринга"),
        ),
        (
            "P005",
            d(2026, 6, 17),
            "data_governance",
            "Зафиксировать владельца правил дедупликации и критерии качества золотой записи.",
            "Ольга Беляева",
            "approved",
            find_id(milestones, project_id="P005", name="Золотая запись"),
        ),
    ]

    rows = []
    for idx, (project_id, decision_date, decision_type, description, owner, status, milestone_id) in enumerate(
        specs,
        start=1,
    ):
        rows.append(
            {
                "id": f"DEC{idx:03d}",
                "project_id": project_id,
                "decision_date": iso(decision_date),
                "decision_type": decision_type,
                "description": description,
                "decision_owner": owner,
                "status": status,
                "linked_milestone_id": milestone_id,
            }
        )
    return rows


def make_change_requests() -> list[dict[str, Any]]:
    specs = [
        (
            "P001",
            d(2026, 6, 12),
            "Security",
            "compliance_scope",
            "Добавить расширенную модель угроз и дополнительные проверки маскирования данных.",
            16_000_000,
            14,
            "under_review",
        ),
        (
            "P001",
            d(2026, 6, 13),
            "Core Platform",
            "integration_scope",
            "Перенести часть интеграционных сценариев кредитного конвейера на период после пилота.",
            -3_000_000,
            -6,
            "proposed",
        ),
        (
            "P002",
            d(2026, 6, 7),
            "Fraud Platform",
            "performance_tuning",
            "Добавить отдельный спринт на оптимизацию поиска признаков до целевых 120 мс.",
            4_000_000,
            8,
            "under_review",
        ),
        (
            "P004",
            d(2026, 6, 14),
            "Payments",
            "vendor_scope",
            "Закупить дополнительные сертификаты поставщика и расширить тестовый имитатор для шлюза.",
            4_000_000,
            5,
            "approved",
        ),
        (
            "P005",
            d(2026, 6, 17),
            "CRM Analytics",
            "data_quality",
            "Добавить ручную проверку верхних сегментов после дедупликации золотой записи.",
            1_000_000,
            2,
            "approved",
        ),
    ]

    rows = []
    for idx, (
        project_id,
        request_date,
        requested_by,
        change_type,
        description,
        requested_budget_delta,
        requested_timeline_delta_days,
        status,
    ) in enumerate(
        specs,
        start=1,
    ):
        rows.append(
            {
                "id": f"CR{idx:03d}",
                "project_id": project_id,
                "request_date": iso(request_date),
                "requested_by": requested_by,
                "change_type": change_type,
                "description": description,
                "requested_budget_delta": requested_budget_delta,
                "requested_timeline_delta_days": requested_timeline_delta_days,
                "status": status,
            }
        )
    return rows


def make_task_dependencies(tasks_by_project: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    specs = [
        ("P001", 5, 1, "blocks", True, 1, "Security approval is required before integration and pilot activities."),
        ("P001", 6, 2, "blocks", True, 0, "DWH SLA confirmation is needed before feature validation."),
        ("P001", 9, 4, "requires", False, 2, "Rollback procedure depends on validated model documentation."),
        ("P002", 4, 1, "blocks", True, 1, "Integration testing waits for the processing window."),
        ("P002", 5, 2, "blocks", True, 1, "Feature store tuning depends on threat model approval."),
        ("P002", 6, 4, "blocks", True, 0, "Latency optimization depends on a stable processing adapter."),
        ("P003", 6, 1, "related", False, 1, "Mobile onboarding should align with UX approval."),
        ("P003", 8, 6, "blocks", False, 0, "Regression package depends on the stabilized client flow."),
        ("P004", 4, 1, "blocks", True, 1, "Payment routing depends on vendor certificate delivery."),
        ("P004", 8, 2, "blocks", True, 2, "Load testing depends on a completed PCI DSS checklist."),
        ("P005", 4, 1, "blocks", True, 1, "Golden record mapping depends on deduplication rules."),
        ("P005", 8, 4, "related", False, 0, "Cross-sell segments depend on stable profile quality."),
    ]

    rows = []
    for idx, (project_id, task_number, depends_on_number, dependency_type, is_critical, lag_days, reason) in enumerate(
            specs, start=1):
        rows.append(
            {
                "id": f"TD{idx:03d}",
                "project_id": project_id,
                "task_id": task_id(tasks_by_project, project_id, task_number),
                "depends_on_task_id": task_id(tasks_by_project, project_id, depends_on_number),
                "dependency_type": dependency_type,
                "is_critical_path": is_critical,
                "lag_days": lag_days,
                "reason": reason,
            }
        )
    return rows


def make_budget_line_items(budgets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    budget_id = {row["project_id"]: row["id"] for row in budgets}
    specs = [
        ("P001", "Engineering", "Model development", "Risk Models", 10_000_000, 8_500_000),
        ("P001", "Integration", "Core Platform integration", "Core Platform", 14_000_000, 12_000_000),
        ("P001", "Compliance", "Security and approvals", "Security", 16_000_000, 13_000_000),
        ("P001", "Testing", "Pilot rollout", "PMO", 8_000_000, 8_000_000),
        ("P002", "Engineering", "Streaming core", "Fraud Platform", 20_000_000, 18_000_000),
        ("P002", "Integration", "Card processing adapter", "Payments", 18_000_000, 16_000_000),
        ("P002", "Modeling", "Feature store and scoring", "Risk Models", 20_000_000, 14_000_000),
        ("P002", "Compliance", "UAT and sign-off", "Compliance", 14_000_000, 16_000_000),
        ("P003", "UX", "Mobile UX and design", "Mobile", 10_000_000, 8_000_000),
        ("P003", "Product", "Client scenarios", "Mobile", 10_000_000, 7_000_000),
        ("P003", "Analytics", "Event tracking", "CRM Analytics", 12_000_000, 9_000_000),
        ("P003", "Release", "Stabilization", "Quality", 10_000_000, 5_500_000),
        ("P004", "Engineering", "Gateway core", "Payments", 10_000_000, 9_000_000),
        ("P004", "Integration", "SBP and acquiring adapters", "Payments", 12_000_000, 8_000_000),
        ("P004", "Security", "PCI DSS workstream", "Security", 8_000_000, 7_000_000),
        ("P004", "Observability", "Load testing and monitoring", "Platform", 8_000_000, 7_000_000),
        ("P005", "Data Quality", "Golden record and dedupe", "CRM Analytics", 9_000_000, 6_000_000),
        ("P005", "Engineering", "CRM profile API", "CRM Platform", 9_000_000, 6_000_000),
        ("P005", "Governance", "Data quality controls", "Data Platform", 8_000_000, 5_000_000),
        ("P005", "Rollout", "Pilot rollout", "Sales", 8_000_000, 5_500_000),
    ]

    rows = []
    for idx, (project_id, category, item_name, owner_team, planned_amount, actual_amount) in enumerate(specs, start=1):
        rows.append(
            {
                "id": f"BI{idx:03d}",
                "project_id": project_id,
                "budget_id": budget_id[project_id],
                "category": category,
                "item_name": item_name,
                "planned_amount": planned_amount,
                "actual_amount": actual_amount,
                "owner_team": owner_team,
            }
        )
    return rows


def make_communication_messages(communications: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = []
    next_id = 1
    for communication in communications:
        project_id = communication["project_id"]
        comm_id = communication["id"]
        topic = communication["topic"]
        channel = communication["channel"]
        from_team = communication["from_team"]
        to_team = communication["to_team"]
        last_date = parse_date(communication["last_message_date"])
        response_date = parse_date(communication["expected_response_date"])
        linked_task_id = communication["linked_task_id"]
        ensure(last_date is not None, f"Communication {comm_id} has no last_message_date")
        ensure(response_date is not None, f"Communication {comm_id} has no expected_response_date")

        rows.append(
            {
                "id": f"CM{next_id:03d}",
                "project_id": project_id,
                "communication_id": comm_id,
                "message_time": iso_dt(dt(last_date.year, last_date.month, last_date.day, 9, 15)),
                "sender_team": from_team,
                "recipient_team": to_team,
                "channel": channel,
                "message_type": "request",
                "status": "sent",
                "summary": f"Первичный запрос по теме: {topic}",
                "linked_task_id": linked_task_id,
                "is_escalation": False,
            }
        )
        next_id += 1

        if communication["status"] == "responded":
            sender_team, recipient_team, message_type, status, summary, escalation = (
                to_team,
                from_team,
                "response",
                "replied",
                f"Ответ по теме: {topic}",
                False,
            )
        elif communication["status"] == "escalated":
            sender_team, recipient_team, message_type, status, summary, escalation = (
                "PMO",
                to_team,
                "escalation",
                "escalated",
                f"Эскалация по теме: {topic}",
                True,
            )
        elif communication["status"] == "delayed":
            sender_team, recipient_team, message_type, status, summary, escalation = (
                from_team,
                to_team,
                "reminder",
                "waiting",
                f"Напоминание по теме: {topic}",
                False,
            )
        else:
            sender_team, recipient_team, message_type, status, summary, escalation = (
                from_team,
                to_team,
                "follow_up",
                "waiting",
                f"Фоллоу-ап по теме: {topic}",
                False,
            )

        rows.append(
            {
                "id": f"CM{next_id:03d}",
                "project_id": project_id,
                "communication_id": comm_id,
                "message_time": iso_dt(dt(response_date.year, response_date.month, response_date.day, 15, 30)),
                "sender_team": sender_team,
                "recipient_team": recipient_team,
                "channel": channel,
                "message_type": message_type,
                "status": status,
                "summary": summary,
                "linked_task_id": linked_task_id,
                "is_escalation": escalation,
            }
        )
        next_id += 1

    return rows


def make_dependencies(tasks_by_project: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    specs = [
        (
            "P001",
            "internal_system",
            "DWH feature mart",
            "Data Platform",
            d(2026, 6, 7),
            "delayed",
            "high",
            task_id(tasks_by_project, "P001", 2),
        ),
        (
            "P001",
            "approval",
            "Security approval",
            "Security",
            d(2026, 6, 5),
            "blocked",
            "critical",
            task_id(tasks_by_project, "P001", 1),
        ),
        (
            "P001",
            "management_decision",
            "Scope cut decision",
            "PMO",
            d(2026, 6, 18),
            "pending",
            "high",
            task_id(tasks_by_project, "P001", 9),
        ),
        (
            "P002",
            "internal_system",
            "Card processing stream",
            "Card Processing",
            d(2026, 6, 3),
            "blocked",
            "critical",
            task_id(tasks_by_project, "P002", 1),
        ),
        (
            "P002",
            "approval",
            "False positive criteria",
            "Compliance",
            d(2026, 6, 10),
            "delayed",
            "high",
            task_id(tasks_by_project, "P002", 8),
        ),
        (
            "P003",
            "release_window",
            "Pilot release slot",
            "Release Management",
            d(2026, 6, 19),
            "pending",
            "medium",
            task_id(tasks_by_project, "P003", 10),
        ),
        (
            "P004",
            "vendor",
            "SBP test certificates",
            "SBP Vendor",
            d(2026, 6, 8),
            "delayed",
            "critical",
            task_id(tasks_by_project, "P004", 1),
        ),
        (
            "P004",
            "approval",
            "PCI DSS token storage review",
            "Security",
            d(2026, 6, 13),
            "delayed",
            "high",
            task_id(tasks_by_project, "P004", 2),
        ),
        (
            "P005",
            "data_owner",
            "CRM duplicate ownership",
            "Data Owners",
            d(2026, 6, 14),
            "responded",
            "medium",
            task_id(tasks_by_project, "P005", 2),
        ),
        (
            "P005",
            "approval",
            "Profile 360 access matrix",
            "Compliance",
            d(2026, 6, 17),
            "pending",
            "medium",
            task_id(tasks_by_project, "P005", 9),
        ),
    ]

    rows = []
    for idx, (project_id, dependency_type, depends_on, owner_team, expected_date, status, criticality, linked_task_id) in enumerate(
        specs,
        start=1,
    ):
        rows.append(
            {
                "id": f"D{idx:03d}",
                "project_id": project_id,
                "dependency_type": dependency_type,
                "depends_on": depends_on,
                "owner_team": owner_team,
                "expected_date": iso(expected_date),
                "status": status,
                "criticality": criticality,
                "linked_task_id": linked_task_id,
            }
        )
    return rows


def make_decisions(milestones: list[dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        (
            "P001",
            d(2026, 6, 12),
            "scope_change",
            "Перенести часть интеграции на период после пилота, чтобы разблокировать проверку безопасности.",
            "Елена Морозова",
            "pending",
            find_id(milestones, project_id="P001", name="Проверка безопасности и интеграция"),
        ),
        (
            "P001",
            d(2026, 6, 13),
            "budget_reserve",
            "Согласовать дополнительный резерв на доработки по соответствию требованиям или сократить объем пилота.",
            "Управляющий комитет",
            "pending",
            find_id(milestones, project_id="P001", name="Пилот в кредитном конвейере"),
        ),
        (
            "P002",
            d(2026, 6, 7),
            "pilot_scope",
            "Запускать пилот антифрод-решений без части правил до завершения интеграции с процессингом.",
            "Андрей Романов",
            "pending",
            find_id(milestones, project_id="P002", name="Интеграция с процессингом"),
        ),
        (
            "P003",
            d(2026, 6, 18),
            "release_go",
            "Подтвердить окно пилотного релиза после расширенного регрессионного прогона.",
            "Артур Комаров",
            "approved",
            find_id(milestones, project_id="P003", name="Пилотный релиз"),
        ),
        (
            "P004",
            d(2026, 6, 14),
            "vendor_escalation",
            "Эскалировать задержку сертификатов СБП через менеджера поставщика.",
            "Сергей Ковалев",
            "approved",
            find_id(milestones, project_id="P004", name="СБП и адаптеры эквайринга"),
        ),
        (
            "P005",
            d(2026, 6, 17),
            "data_governance",
            "Зафиксировать владельца правил дедупликации и критерии качества золотой записи.",
            "Ольга Беляева",
            "approved",
            find_id(milestones, project_id="P005", name="Золотая запись"),
        ),
    ]

    rows = []
    for idx, (project_id, decision_date, decision_type, description, owner, status, milestone_id) in enumerate(
        specs,
        start=1,
    ):
        rows.append(
            {
                "id": f"DEC{idx:03d}",
                "project_id": project_id,
                "decision_date": iso(decision_date),
                "decision_type": decision_type,
                "description": description,
                "decision_owner": owner,
                "status": status,
                "linked_milestone_id": milestone_id,
            }
        )
    return rows


def make_change_requests() -> list[dict[str, Any]]:
    specs = [
        (
            "P001",
            d(2026, 6, 12),
            "Security",
            "compliance_scope",
            "Добавить расширенную модель угроз и дополнительные проверки маскирования данных.",
            16_000_000,
            14,
            "under_review",
        ),
        (
            "P001",
            d(2026, 6, 13),
            "Core Platform",
            "integration_scope",
            "Перенести часть интеграционных сценариев кредитного конвейера на период после пилота.",
            -3_000_000,
            -6,
            "proposed",
        ),
        (
            "P002",
            d(2026, 6, 7),
            "Fraud Platform",
            "performance_tuning",
            "Добавить отдельный спринт на оптимизацию поиска признаков до целевых 120 мс.",
            4_000_000,
            8,
            "under_review",
        ),
        (
            "P004",
            d(2026, 6, 14),
            "Payments",
            "vendor_scope",
            "Закупить дополнительные сертификаты поставщика и расширить тестовый имитатор для шлюза.",
            4_000_000,
            5,
            "approved",
        ),
        (
            "P005",
            d(2026, 6, 17),
            "CRM Analytics",
            "data_quality",
            "Добавить ручную проверку верхних сегментов после дедупликации золотой записи.",
            1_000_000,
            2,
            "approved",
        ),
    ]

    rows = []
    for idx, (
        project_id,
        request_date,
        requested_by,
        change_type,
        description,
        requested_budget_delta,
        requested_timeline_delta_days,
        status,
    ) in enumerate(
        specs,
        start=1,
    ):
        rows.append(
            {
                "id": f"CR{idx:03d}",
                "project_id": project_id,
                "request_date": iso(request_date),
                "requested_by": requested_by,
                "change_type": change_type,
                "description": description,
                "requested_budget_delta": requested_budget_delta,
                "requested_timeline_delta_days": requested_timeline_delta_days,
                "status": status,
            }
        )
    return rows


def localized_csv_value(value: Any) -> Any:
    if isinstance(value, bool):
        return "истина" if value else "ложь"
    return value


def write_csv(filename: str, rows: list[dict[str, Any]], columns: list[str]) -> None:
    DATA_DIR.mkdir(exist_ok=True)
    localized_rows = [
        {CSV_COLUMN_LABELS[column]: localized_csv_value(row.get(column, "")) for column in columns}
        for row in rows
    ]
    pd.DataFrame(
        localized_rows,
        columns=[CSV_COLUMN_LABELS[column] for column in columns],
    ).to_csv(DATA_DIR / filename, index=False)


def remove_stale_derived_files() -> None:
    for filename in [
        "metrics_snapshots.csv",
        "project_events.csv",
        "agents_json/batch_output.json",
        "agents_json/project_monitoring_output.json",
    ]:
        path = DATA_DIR / filename
        if path.exists():
            path.unlink()


def forecast_total_spent_from_source(
    project_id: str,
    budget: dict[str, Any],
    budget_line_items: list[dict[str, Any]],
    change_requests: list[dict[str, Any]],
) -> int:
    base_forecast = sum(
        max(item["planned_amount"], item["actual_amount"])
        for item in budget_line_items
        if item["project_id"] == project_id
    )
    if base_forecast == 0:
        base_forecast = max(budget["planned_budget"], budget["actual_spent"])

    requested_budget_delta = sum(
        request["requested_budget_delta"]
        for request in change_requests
        if request["project_id"] == project_id
        and request["status"] in {"pending", "under_review", "proposed", "approved"}
    )
    return max(budget["actual_spent"], base_forecast + requested_budget_delta)


def next_id(rows: list[dict[str, Any]], prefix: str) -> int:
    return max((int(row["id"].removeprefix(prefix)) for row in rows), default=0) + 1


def split_total(total: int, count: int, base: int) -> list[int]:
    weights = [base + (idx % 11) * base // 7 + (idx % 5) * base // 9 for idx in range(count)]
    weight_sum = sum(weights)
    amounts = [total * weight // weight_sum for weight in weights]
    remainder = total - sum(amounts)
    amounts[-1] += remainder
    return amounts


def append_max2_project(
    *,
    projects: list[dict[str, Any]],
    resources: list[dict[str, Any]],
    allocations: list[dict[str, Any]],
    tasks: list[dict[str, Any]],
    milestones: list[dict[str, Any]],
    budgets: list[dict[str, Any]],
    risks: list[dict[str, Any]],
    communications: list[dict[str, Any]],
    task_dependencies: list[dict[str, Any]],
    budget_line_items: list[dict[str, Any]],
    communication_messages: list[dict[str, Any]],
    task_history: list[dict[str, Any]],
    task_comments: list[dict[str, Any]],
    dependencies: list[dict[str, Any]],
    decisions: list[dict[str, Any]],
    change_requests: list[dict[str, Any]],
) -> None:
    project_id = "P006"
    if any(project["id"] == project_id for project in projects):
        return

    projects.append(
        {
            "id": project_id,
            "name": "MAX2.0",
            "lifecycle_status": "active",
            "priority": "critical",
            "start_date": iso(d(2026, 5, 25)),
            "planned_end_date": iso(d(2026, 8, 14)),
            "business_goal": "Собрать компактную банковскую платформу MAX2.0 для клиентских сервисов, платежей и управления рисками.",
            "expected_result": "Рабочее ядро платформы, интеграционный слой, витрина данных, мониторинг и пилотная миграция ключевых процессов.",
            "business_value": "Быстрый запуск новых банковских продуктов и снижение затрат на поддержку разрозненных сервисов.",
        }
    )

    roles = [
        ("Project Manager", "MAX PMO", 40, 5200, "senior"),
        ("Business Analyst", "MAX Analysis", 40, 3900, "middle+"),
        ("Solution Architect", "MAX Architecture", 32, 6100, "principal"),
        ("Backend Developer", "MAX Core", 40, 5000, "senior"),
        ("Frontend Developer", "MAX Front", 40, 4300, "middle+"),
        ("Data Engineer", "MAX Data", 40, 4800, "senior"),
        ("ML Engineer", "MAX AI", 40, 5200, "senior"),
        ("QA Automation Engineer", "MAX Quality", 40, 3900, "middle+"),
        ("DevOps Engineer", "MAX Platform", 40, 5000, "senior"),
        ("Security Engineer", "MAX Security", 32, 5600, "senior"),
        ("Integration Engineer", "MAX Integration", 40, 4700, "senior"),
        ("SRE", "MAX Reliability", 40, 5100, "senior"),
        ("Compliance Officer", "MAX Compliance", 32, 4300, "senior"),
        ("UX Researcher", "MAX CX", 32, 3600, "middle"),
    ]
    surnames = [
        "Аверин",
        "Басова",
        "Воронцов",
        "Галкин",
        "Демина",
        "Еремин",
        "Жукова",
        "Зорин",
        "Исаев",
        "Климова",
        "Ларионов",
        "Медведева",
        "Назаров",
        "Орлова",
        "Петров",
        "Рябова",
        "Сафонов",
        "Тихонова",
        "Устинов",
        "Филатова",
    ]
    first_names = [
        "Артем",
        "Валерия",
        "Глеб",
        "Диана",
        "Егор",
        "Инна",
        "Лев",
        "Милана",
        "Петр",
        "Яна",
    ]

    resource_start = next_id(resources, "R")
    max_resource_ids: list[str] = []
    for offset in range(8):
        role, team, capacity, rate, seniority = roles[offset % len(roles)]
        resource_id = f"R{resource_start + offset:03d}"
        max_resource_ids.append(resource_id)
        resources.append(
            {
                "id": resource_id,
                "full_name": f"{first_names[offset % len(first_names)]} {surnames[offset % len(surnames)]}",
                "role": role,
                "team": team,
                "available_hours_per_week": capacity,
                "hour_rate": rate + (offset % 7) * 150,
                "seniority": seniority,
            }
        )

    allocation_start = next_id(allocations, "RA")
    for offset, resource_id in enumerate(max_resource_ids):
        resource = resources[resource_start - 1 + offset]
        capacity = resource["available_hours_per_week"]
        planned = min(capacity, 24 + (offset % 5) * 4)
        actual = min(int(capacity * 1.45), planned + 4 + (offset % 6) * 3)
        allocations.append(
            {
                "id": f"RA{allocation_start + offset:03d}",
                "resource_id": resource_id,
                "project_id": project_id,
                "planned_hours_per_week": planned,
                "actual_hours_per_week": actual,
            }
        )

    task_start = next_id(tasks, "T")
    task_subjects = [
        "контур идентификации клиента",
        "платежный слой оркестрации",
        "единый профиль клиента",
        "витрину клиентских событий",
        "онлайн-антифрод",
        "кредитный механизм принятия решений",
        "корпоративный шлюз программных интерфейсов",
        "модуль лимитов и тарифов",
        "операционный мониторинг",
        "сервис уведомлений",
        "конвейер качества данных",
        "миграцию устаревших процессов",
        "контур аудита действий",
        "ролевую модель доступа",
        "ИИ-помощника оператора",
        "модуль согласований",
        "каталог банковских продуктов",
        "шину интеграций",
        "наблюдаемость платформы",
        "контур аварийного восстановления",
    ]
    domains = ["розницы", "МСБ", "корпоративного блока", "рисков", "платежей", "контакт-центра", "операционного офиса"]
    max_tasks: list[dict[str, Any]] = []
    for number in range(1, 25):
        assignee_id = max_resource_ids[number % len(max_resource_ids)]
        assignee = next(resource for resource in resources if resource["id"] == assignee_id)
        is_blocked = number <= 2
        is_overdue_open = 2 < number <= 7
        is_done = 7 < number <= 20

        if is_blocked:
            status = "Blocked"
            planned_due = LATEST_SNAPSHOT_DATE - timedelta(days=1 + number % 45)
            actual_end = None
            blocker_reason = [
                "Не согласован целевой контракт программного интерфейса со смежной системой",
                "Ожидается согласование безопасности для продуктивного доступа",
                "Не подтверждено окно миграции устаревших данных",
                "Нужен владелец бизнес-правила для спорного сценария",
                "Инфраструктурный лимит не позволяет поднять тестовый стенд",
            ][number % 5]
            priority = "critical" if number == 1 else "high"
            spent_hours = 24 + number % 150
        elif is_overdue_open:
            status = ["In Progress", "Review", "In Progress", "Review"][number % 4]
            planned_due = LATEST_SNAPSHOT_DATE - timedelta(days=1 + number % 70)
            actual_end = None
            blocker_reason = ""
            priority = ["critical", "high", "high", "medium"][number % 4]
            spent_hours = 12 + number % 130
        elif is_done:
            status = "Done"
            planned_due = d(2026, 5, 30) + timedelta(days=number % 230)
            actual_end = planned_due + timedelta(days=(number % 17) - 4)
            if actual_end > LATEST_SNAPSHOT_DATE:
                actual_end = LATEST_SNAPSHOT_DATE - timedelta(days=number % 4)
            blocker_reason = ""
            priority = ["high", "medium", "medium", "low"][number % 4]
            spent_hours = 8 + number % 100
        else:
            status = ["To Do", "In Progress", "To Do"][number % 3]
            planned_due = LATEST_SNAPSHOT_DATE + timedelta(days=1 + number % 180)
            actual_end = None
            blocker_reason = ""
            priority = ["high", "medium", "low", "medium"][number % 4]
            spent_hours = number % 50

        task = {
            "id": f"T{task_start + number - 1:03d}",
            "project_id": project_id,
            "external_id": f"MAX20-{number:04d}",
            "title": f"{TASK_ACTIONS[number % len(TASK_ACTIONS)]} {task_subjects[number % len(task_subjects)]} для {domains[number % len(domains)]}",
            "assignee_id": assignee_id,
            "assignee_name": assignee["full_name"],
            "status": status,
            "priority": priority,
            "planned_due_date": iso(planned_due),
            "actual_end_date": iso(actual_end),
            "estimated_hours": max(8, spent_hours + ((number % 41) - 12)),
            "spent_hours": spent_hours,
            "is_blocked": is_blocked,
            "blocker_reason": blocker_reason,
        }
        tasks.append(task)
        max_tasks.append(task)

    milestone_start = next_id(milestones, "M")
    for offset in range(4):
        planned_start = d(2026, 5, 25) + timedelta(days=offset * 11)
        planned_end = planned_start + timedelta(days=10 + offset % 9)
        if offset == 0:
            actual_start = planned_start + timedelta(days=offset % 3)
            actual_end = planned_end + timedelta(days=(offset % 5) - 1)
            status = "Done"
        elif offset == 1:
            actual_start = planned_start + timedelta(days=offset % 4)
            actual_end = None
            status = "Delayed" if offset % 2 else "At Risk"
        elif offset == 2:
            actual_start = planned_start if planned_start <= LATEST_SNAPSHOT_DATE else None
            actual_end = None
            status = "In Progress"
        else:
            actual_start = None
            actual_end = None
            status = "Planned"
        milestones.append(
            {
                "id": f"M{milestone_start + offset:03d}",
                "project_id": project_id,
                "name": f"MAX2.0 этап {offset + 1}",
                "planned_start_date": iso(planned_start),
                "planned_end_date": iso(planned_end),
                "actual_start_date": iso(actual_start),
                "actual_end_date": iso(actual_end),
                "status": status,
                "responsible_team": roles[offset % len(roles)][1],
            }
        )

    budget_id = f"B{next_id(budgets, 'B'):03d}"
    planned_budget = 48_000_000
    actual_spent = 39_000_000
    budgets.append(
        {
            "id": budget_id,
            "project_id": project_id,
            "planned_budget": planned_budget,
            "actual_spent": actual_spent,
            "expected_economic_effect": 92_000_000,
            "cost_of_delay_per_day": 850_000,
            "currency": "RUB",
        }
    )

    budget_item_start = next_id(budget_line_items, "BI")
    planned_amounts = split_total(planned_budget, 4, 6_000_000)
    actual_amounts = split_total(actual_spent, 4, 5_000_000)
    budget_categories = [
        "Платформенное ядро",
        "Интеграция",
        "Данные",
        "ИИ",
    ]
    for offset, (planned_amount, actual_amount) in enumerate(zip(planned_amounts, actual_amounts), start=0):
        budget_line_items.append(
            {
                "id": f"BI{budget_item_start + offset:03d}",
                "project_id": project_id,
                "budget_id": budget_id,
                "category": budget_categories[offset % len(budget_categories)],
                "item_name": f"MAX2.0 статья {offset + 1:02d}",
                "planned_amount": planned_amount,
                "actual_amount": actual_amount,
                "owner_team": roles[offset % len(roles)][1],
            }
        )

    risk_start = next_id(risks, "RK")
    risk_types = ["Безопасность", "Интеграция", "Качество данных", "Миграция"]
    risk_statuses = ["active", "mitigating", "active", "escalated"]
    for offset in range(4):
        linked_task = max_tasks[(offset * 7) % len(max_tasks)]
        risks.append(
            {
                "id": f"RK{risk_start + offset:03d}",
                "project_id": project_id,
                "risk_type": risk_types[offset % len(risk_types)],
                "description": f"MAX2.0 риск {offset + 1}: возможное влияние на поток {offset % 24 + 1} из-за зависимости или качества данных.",
                "probability": 2 + offset % 4,
                "impact": 3 + offset % 3,
                "owner_name": resources[resource_start - 1 + offset % len(max_resource_ids)]["full_name"],
                "mitigation_plan": "Назначить ответственного, подтвердить дату снятия риска и подготовить резервный план.",
                "status": risk_statuses[offset % len(risk_statuses)],
                "linked_task_id": linked_task["id"],
            }
        )

    communication_start = next_id(communications, "C")
    message_start = next_id(communication_messages, "CM")
    communication_statuses = ["pending", "delayed", "responded", "escalated"]
    for offset in range(4):
        task = max_tasks[(offset * 5) % len(max_tasks)]
        from_team = roles[offset % len(roles)][1]
        to_team = roles[(offset + 3) % len(roles)][1]
        last_date = LATEST_SNAPSHOT_DATE - timedelta(days=offset % 21)
        expected_date = LATEST_SNAPSHOT_DATE - timedelta(days=(offset % 17) - 5)
        status = communication_statuses[offset % len(communication_statuses)]
        comm_id = f"C{communication_start + offset:03d}"
        communications.append(
            {
                "id": comm_id,
                "project_id": project_id,
                "from_team": from_team,
                "to_team": to_team,
                "topic": f"MAX2.0 согласование потока {offset % 30 + 1}",
                "channel": ["Teams", "Email", "Jira", "Confluence"][offset % 4],
                "last_message_date": iso(last_date),
                "expected_response_date": iso(expected_date),
                "status": status,
                "importance": ["critical", "high", "medium", "low"][offset % 4],
                "linked_task_id": task["id"],
            }
        )
        communication_messages.append(
            {
                "id": f"CM{message_start + offset * 2:03d}",
                "project_id": project_id,
                "communication_id": comm_id,
                "message_time": iso_dt(dt(last_date.year, last_date.month, last_date.day, 10, offset % 60)),
                "sender_team": from_team,
                "recipient_team": to_team,
                "channel": ["Teams", "Email", "Jira", "Confluence"][offset % 4],
                "message_type": "request",
                "status": "sent",
                "summary": f"Первичный запрос MAX2.0 по согласованию {offset % 30 + 1}.",
                "linked_task_id": task["id"],
                "is_escalation": False,
            }
        )
        communication_messages.append(
            {
                "id": f"CM{message_start + offset * 2 + 1:03d}",
                "project_id": project_id,
                "communication_id": comm_id,
                "message_time": iso_dt(dt(expected_date.year, expected_date.month, expected_date.day, 16, offset % 60)),
                "sender_team": "PMO" if status == "escalated" else to_team,
                "recipient_team": to_team if status == "escalated" else from_team,
                "channel": ["Teams", "Email", "Jira", "Confluence"][offset % 4],
                "message_type": "escalation" if status == "escalated" else "response",
                "status": "escalated" if status == "escalated" else "waiting",
                "summary": f"Ответ или эскалация MAX2.0 по согласованию {offset % 30 + 1}.",
                "linked_task_id": task["id"],
                "is_escalation": status == "escalated",
            }
        )

    task_dependency_start = next_id(task_dependencies, "TD")
    for offset in range(4):
        task = max_tasks[offset % len(max_tasks)]
        upstream = max_tasks[(offset * 3 + 17) % len(max_tasks)]
        if upstream["id"] == task["id"]:
            upstream = max_tasks[(offset + 1) % len(max_tasks)]
        task_dependencies.append(
            {
                "id": f"TD{task_dependency_start + offset:03d}",
                "project_id": project_id,
                "task_id": task["id"],
                "depends_on_task_id": upstream["id"],
                "dependency_type": ["blocks", "requires", "related"][offset % 3],
                "is_critical_path": offset % 5 == 0,
                "lag_days": offset % 6,
                "reason": f"MAX2.0 зависимость между потоками {offset % 40 + 1}.",
            }
        )

    history_start = next_id(task_history, "TH")
    for offset in range(6):
        task = max_tasks[offset % len(max_tasks)]
        changed_at = dt(2026, 6, 1 + offset % 18, 9 + offset % 9, offset % 60)
        field = ["status", "planned_due_date", "estimated_hours", "spent_hours", "assignee_id"][offset % 5]
        if field == "status":
            old_value = "To Do"
            new_value = task["status"]
        elif field == "planned_due_date":
            old_value = iso(parse_date(task["planned_due_date"]) - timedelta(days=5))
            new_value = task["planned_due_date"]
        elif field == "estimated_hours":
            old_value = str(max(8, task["estimated_hours"] - 8))
            new_value = str(task["estimated_hours"])
        elif field == "spent_hours":
            old_value = str(max(0, task["spent_hours"] - 6))
            new_value = str(task["spent_hours"])
        else:
            old_value = max_resource_ids[(offset + 1) % len(max_resource_ids)]
            new_value = task["assignee_id"]
        task_history.append(
            {
                "id": f"TH{history_start + offset:03d}",
                "project_id": project_id,
                "task_id": task["id"],
                "changed_at": iso_dt(changed_at),
                "field_changed": field,
                "old_value": old_value,
                "new_value": new_value,
                "changed_by": max_resource_ids[offset % len(max_resource_ids)],
                "source_system": "tasktracker",
            }
        )

    comment_start = next_id(task_comments, "TC")
    comment_templates = [
        "Нужен владелец решения по межсистемному контракту.",
        "Подтверждаем проблему на интеграционном стенде, нужен повторный прогон.",
        "Риски по миграции данных остаются открытыми до сверки контрольной выборки.",
        "Команда просит зафиксировать приоритет потока на ближайший sync.",
        "Согласование зависло у внешней команды, готовим эскалацию.",
    ]
    for offset in range(6):
        task = max_tasks[(offset * 2) % len(max_tasks)]
        author_id = max_resource_ids[offset % len(max_resource_ids)]
        author = next(resource for resource in resources if resource["id"] == author_id)
        created_at = dt(2026, 6, 1 + offset % 18, 8 + offset % 10, offset % 60)
        task_comments.append(
            {
                "id": f"TC{comment_start + offset:03d}",
                "project_id": project_id,
                "task_id": task["id"],
                "author_id": author_id,
                "author_name": author["full_name"],
                "created_at": iso_dt(created_at),
                "channel": ["Jira", "Teams", "Email", "Confluence"][offset % 4],
                "text": f"MAX2.0: {comment_templates[offset % len(comment_templates)]}",
                "mentions_count": offset % 4,
                "source_system": "tasktracker",
            }
        )

    dependency_start = next_id(dependencies, "D")
    for offset in range(2):
        task = max_tasks[(offset * 11) % len(max_tasks)]
        dependencies.append(
            {
                "id": f"D{dependency_start + offset:03d}",
                "project_id": project_id,
                "dependency_type": ["internal_system", "approval", "vendor", "release_window", "data_owner"][offset % 5],
                "depends_on": f"MAX2.0 внешний контур {offset % 45 + 1}",
                "owner_team": roles[(offset + 4) % len(roles)][1],
                "expected_date": iso(LATEST_SNAPSHOT_DATE - timedelta(days=(offset % 22) - 4)),
                "status": ["pending", "delayed", "blocked", "responded"][offset % 4],
                "criticality": ["critical", "high", "medium", "low"][offset % 4],
                "linked_task_id": task["id"],
            }
        )

    decision_start = next_id(decisions, "DEC")
    max_milestones = [milestone for milestone in milestones if milestone["project_id"] == project_id]
    for offset in range(2):
        milestone = max_milestones[offset % len(max_milestones)]
        decisions.append(
            {
                "id": f"DEC{decision_start + offset:03d}",
                "project_id": project_id,
                "decision_date": iso(d(2026, 6, 1) + timedelta(days=offset % 40)),
                "decision_type": ["scope", "architecture", "budget", "security", "migration", "release"][offset % 6],
                "description": f"MAX2.0 управленческое решение {offset + 1} по потоку {offset % 30 + 1}.",
                "decision_owner": ["Максим Туревич", "Steering Committee", "PMO", "Архитектурный комитет"][offset % 4],
                "status": ["pending", "under_review", "approved", "pending"][offset % 4],
                "linked_milestone_id": milestone["id"],
            }
        )

    change_request_start = next_id(change_requests, "CR")
    for offset in range(2):
        change_requests.append(
            {
                "id": f"CR{change_request_start + offset:03d}",
                "project_id": project_id,
                "request_date": iso(d(2026, 6, 1) + timedelta(days=offset % 45)),
                "requested_by": roles[offset % len(roles)][1],
                "change_type": ["integration_scope", "security_scope", "migration_scope", "performance_tuning", "release_scope"][offset % 5],
                "description": f"MAX2.0 запрос на изменение {offset + 1}: уточнение объема работ по крупному потоку.",
                "requested_budget_delta": [-20_000_000, 15_000_000, 35_000_000, 0, 50_000_000][offset % 5],
                "requested_timeline_delta_days": [-5, 3, 8, 0, 12][offset % 5],
                "status": ["pending", "under_review", "proposed", "approved", "rejected"][offset % 5],
            }
        )


def validate_dataset(
    projects: list[dict[str, Any]],
    tasks: list[dict[str, Any]],
    task_dependencies: list[dict[str, Any]],
    budget_line_items: list[dict[str, Any]],
    communication_messages: list[dict[str, Any]],
    task_history: list[dict[str, Any]],
    task_comments: list[dict[str, Any]],
    dependencies: list[dict[str, Any]],
    decisions: list[dict[str, Any]],
    change_requests: list[dict[str, Any]],
    resources: list[dict[str, Any]],
    allocations: list[dict[str, Any]],
    budgets: list[dict[str, Any]],
    milestones: list[dict[str, Any]],
    risks: list[dict[str, Any]],
    communications: list[dict[str, Any]],
) -> None:
    ensure(len(tasks) >= 100, f"Expected at least 100 tasks, got {len(tasks)}")
    max2_tasks_count = sum(1 for task in tasks if task["project_id"] == "P006")
    ensure(
        20 <= max2_tasks_count <= 35,
        f"Expected MAX2.0 to be comparable to other projects, got {max2_tasks_count} tasks",
    )
    ensure(
        all(project["lifecycle_status"] in {"active", "paused", "completed", "cancelled"} for project in projects),
        "Projects must keep only lifecycle source status, not calculated risk color",
    )
    ensure(any(task["is_blocked"] for task in tasks), "Expected at least one blocked task")

    has_overdue_open_task = False
    for task in tasks:
        planned_due = parse_date(task["planned_due_date"])
        if task["status"] != "Done" and planned_due is not None and planned_due < LATEST_SNAPSHOT_DATE:
            has_overdue_open_task = True
            break
    ensure(has_overdue_open_task, "Expected at least one overdue open task")

    availability = {resource["id"]: resource["available_hours_per_week"] for resource in resources}
    total_actual_by_resource: dict[str, float] = defaultdict(float)
    for allocation in allocations:
        total_actual_by_resource[allocation["resource_id"]] += allocation["actual_hours_per_week"]
    ensure(bool(total_actual_by_resource), "Expected resource allocations")

    max_total_allocation = max(
        total_actual_by_resource[resource_id] / availability[resource_id] * 100
        for resource_id in total_actual_by_resource
    )
    ensure(
        130 <= max_total_allocation <= 150,
        f"Expected max resource allocation between 130 and 150%, got {max_total_allocation:.1f}%",
    )

    valid_project_ids = {project["id"] for project in projects}
    valid_task_ids = {task["id"] for task in tasks}
    invalid_dependencies = [
        dep["id"]
        for dep in task_dependencies
        if dep["task_id"] not in valid_task_ids or dep["depends_on_task_id"] not in valid_task_ids
    ]
    self_dependencies = [
        dep["id"]
        for dep in task_dependencies
        if dep["task_id"] == dep["depends_on_task_id"]
    ]
    ensure(len(task_dependencies) >= 10, "Expected at least 10 task dependencies")
    ensure(not invalid_dependencies, f"Dependencies with invalid task ids: {invalid_dependencies}")
    ensure(not self_dependencies, f"Dependencies cannot point to the same task: {self_dependencies}")

    budget_totals: dict[str, dict[str, int]] = defaultdict(lambda: {"planned": 0, "actual": 0})
    for item in budget_line_items:
        budget_totals[item["project_id"]]["planned"] += item["planned_amount"]
        budget_totals[item["project_id"]]["actual"] += item["actual_amount"]
    for budget in budgets:
        project_id = budget["project_id"]
        totals = budget_totals[budget["project_id"]]
        ensure(
            totals["planned"] == budget["planned_budget"],
            f"Budget planned mismatch for {project_id}: {totals['planned']} != {budget['planned_budget']}",
        )
        ensure(
            totals["actual"] == budget["actual_spent"],
            f"Budget actual mismatch for {project_id}: {totals['actual']} != {budget['actual_spent']}",
        )

    valid_communication_ids = {communication["id"] for communication in communications}
    invalid_message_ids = [
        message["id"]
        for message in communication_messages
        if message["communication_id"] not in valid_communication_ids
    ]
    ensure(
        len(communication_messages) == len(communications) * 2,
        f"Expected two messages per communication, got {len(communication_messages)} messages",
    )
    ensure(not invalid_message_ids, f"Messages with invalid communication ids: {invalid_message_ids}")

    invalid_history_ids = [
        item["id"]
        for item in task_history
        if item["project_id"] not in valid_project_ids or item["task_id"] not in valid_task_ids
    ]
    ensure(len(task_history) >= 15, "Expected at least 15 task history records")
    ensure(not invalid_history_ids, f"Task history records with invalid references: {invalid_history_ids}")
    ensure(
        any(
            item["project_id"] == "P001"
            and item["task_id"] == task_id(task_lookup(tasks), "P001", 1)
            and item["field_changed"] == "status"
            and item["new_value"] == "Blocked"
            for item in task_history
        ),
        "Expected task history to include P001 Security task blocking",
    )

    invalid_comment_ids = [
        item["id"]
        for item in task_comments
        if item["project_id"] not in valid_project_ids or item["task_id"] not in valid_task_ids
    ]
    ensure(len(task_comments) >= 15, "Expected at least 15 task comments")
    ensure(not invalid_comment_ids, f"Task comments with invalid references: {invalid_comment_ids}")
    ensure(
        any(item["project_id"] == "P001" and "Безопасность" in item["text"] for item in task_comments),
        "Expected task comments to include P001 Security discussion",
    )

    negative_roi_projects = [
        budget
        for budget in budgets
        if (
            budget["expected_economic_effect"]
            - forecast_total_spent_from_source(
                budget["project_id"],
                budget,
                budget_line_items,
                change_requests,
            )
        )
        / forecast_total_spent_from_source(
            budget["project_id"],
            budget,
            budget_line_items,
            change_requests,
        )
        < 0
    ]
    ensure(bool(negative_roi_projects), "Expected at least one project with negative ROI")
    ensure(
        any(milestone["project_id"] == "P001" and milestone["status"] == "Delayed" for milestone in milestones),
        "Expected a delayed milestone for P001",
    )
    ensure(
        any(risk["project_id"] == "P001" and risk["probability"] * risk["impact"] >= 15 for risk in risks),
        "Expected a high-risk item for P001",
    )
    ensure(
        any(
            communication["project_id"] == "P001" and communication["status"] == "escalated"
            for communication in communications
        ),
        "Expected an escalated communication for P001",
    )

    valid_milestone_ids = {milestone["id"] for milestone in milestones}

    ensure(len(dependencies) >= 8, "Expected at least 8 project dependencies")
    ensure(
        any(
            dep["project_id"] == "P001"
            and dep["criticality"] == "critical"
            and dep["status"] in {"blocked", "delayed"}
            for dep in dependencies
        ),
        "Expected a critical blocked or delayed dependency for P001",
    )
    ensure(
        all(dep["project_id"] in valid_project_ids and dep["linked_task_id"] in valid_task_ids for dep in dependencies),
        "Dependencies must reference existing projects and tasks",
    )

    ensure(len(decisions) >= 5, "Expected at least 5 project decisions")
    ensure(
        any(decision["project_id"] == "P001" and decision["status"] == "pending" for decision in decisions),
        "Expected a pending decision for P001",
    )
    ensure(
        all(
            decision["project_id"] in valid_project_ids
            and decision["linked_milestone_id"] in valid_milestone_ids
            for decision in decisions
        ),
        "Decisions must reference existing projects and milestones",
    )

    ensure(len(change_requests) >= 5, "Expected at least 5 change requests")
    ensure(
        any(
            request["project_id"] == "P001"
            and request["requested_budget_delta"] > 0
            and request["requested_timeline_delta_days"] > 0
            for request in change_requests
        ),
        "Expected a positive requested budget and timeline change request for P001",
    )
    ensure(
        all(request["project_id"] in valid_project_ids for request in change_requests),
        "Change requests must reference existing projects",
    )


TECHNICAL_VALUE_FIELDS = {
    "id",
    "project_id",
    "external_id",
    "task_id",
    "depends_on_task_id",
    "assignee_id",
    "author_id",
    "changed_by",
    "resource_id",
    "budget_id",
    "communication_id",
    "linked_task_id",
    "linked_milestone_id",
}

EXACT_RU_VALUES = {
    "active": "активен",
    "paused": "на паузе",
    "completed": "завершен",
    "cancelled": "отменен",
    "critical": "критический",
    "high": "высокий",
    "medium": "средний",
    "low": "низкий",
    "Blocked": "Заблокирована",
    "In Progress": "В работе",
    "Review": "На проверке",
    "Done": "Завершена",
    "To Do": "Запланирована",
    "Delayed": "Задерживается",
    "At Risk": "Под риском",
    "Planned": "Запланирована",
    "mitigating": "снижается",
    "escalated": "эскалировано",
    "pending": "ожидает",
    "delayed": "задерживается",
    "blocked": "заблокировано",
    "responded": "получен ответ",
    "under_review": "на рассмотрении",
    "proposed": "предложено",
    "approved": "согласовано",
    "rejected": "отклонено",
    "request": "запрос",
    "response": "ответ",
    "escalation": "эскалация",
    "reminder": "напоминание",
    "follow_up": "контрольный запрос",
    "sent": "отправлено",
    "waiting": "ожидает",
    "replied": "получен ответ",
    "blocks": "блокирует",
    "requires": "требует",
    "related": "связана",
    "internal_system": "внутренняя система",
    "approval": "согласование",
    "management_decision": "управленческое решение",
    "release_window": "релизное окно",
    "data_owner": "владелец данных",
    "vendor": "поставщик",
    "scope_change": "изменение объема",
    "budget_reserve": "резерв бюджета",
    "pilot_scope": "объем пилота",
    "release_go": "разрешение на релиз",
    "vendor_escalation": "эскалация поставщика",
    "data_governance": "управление данными",
    "architecture": "архитектура",
    "compliance_scope": "объем проверок соответствия",
    "integration_scope": "объем интеграции",
    "performance_tuning": "оптимизация производительности",
    "vendor_scope": "объем работ поставщика",
    "data_quality": "качество данных",
    "security_scope": "объем работ по безопасности",
    "migration_scope": "объем миграции",
    "release_scope": "объем релиза",
    "tasktracker": "трекер задач",
    "RUB": "руб.",
    "Email": "Почта",
    "Teams": "Команды",
    "Jira": "Трекер задач",
    "Confluence": "База знаний",
    "senior": "старший",
    "middle+": "средний+",
    "middle": "средний",
    "principal": "ведущий эксперт",
    "Project Manager": "Руководитель проектов",
    "Business Analyst": "Бизнес-аналитик",
    "Solution Architect": "Архитектор решений",
    "Backend Developer": "Бэкенд-разработчик",
    "Frontend Developer": "Фронтенд-разработчик",
    "Data Engineer": "Инженер данных",
    "ML Engineer": "Инженер машинного обучения",
    "QA Automation Engineer": "Инженер автоматизации тестирования",
    "DevOps Engineer": "Инженер платформы",
    "Security Engineer": "Инженер безопасности",
    "Integration Engineer": "Инженер интеграций",
    "Compliance Officer": "Специалист по соответствию",
    "UX Researcher": "Исследователь пользовательского опыта",
    "SRE": "Инженер надежности",
    "QA Engineer": "Инженер тестирования",
    "Data Scientist": "Специалист по данным",
    "Data Analyst": "Аналитик данных",
    "Android Developer": "Android-разработчик",
    "iOS Developer": "iOS-разработчик",
    "API Analyst": "Аналитик программных интерфейсов",
    "Product Owner": "Владелец продукта",
    "PMO": "Проектный офис",
    "Business Analysis": "Бизнес-анализ",
    "Risk Models": "Риск-модели",
    "Core Platform": "Платформенное ядро",
    "Security": "Безопасность",
    "Data Platform": "Платформа данных",
    "Credit Conveyor": "Кредитный конвейер",
    "Fraud Platform": "Антифрод-платформа",
    "Card Processing": "Карточный процессинг",
    "Compliance": "Соответствие требованиям",
    "Platform": "Платформа",
    "Payments": "Платежи",
    "Mobile": "Мобильная команда",
    "Analytics": "Аналитика",
    "Digital Bank": "Цифровой банк",
    "Quality": "Качество",
    "CX": "Клиентский опыт",
    "Release Management": "Управление релизами",
    "Integration": "Интеграция",
    "SBP Vendor": "Поставщик СБП",
    "SRE": "Надежность",
    "CRM Analytics": "CRM-аналитика",
    "Data Owners": "Владельцы данных",
    "CRM Platform": "CRM-платформа",
    "Sales": "Продажи",
    "Business Owner": "Бизнес-заказчик",
    "Enterprise Architecture": "Корпоративная архитектура",
    "Mobile Web": "Мобильный веб",
    "MAX PMO": "MAX проектный офис",
    "MAX Analysis": "MAX аналитика",
    "MAX Architecture": "MAX архитектура",
    "MAX Core": "MAX ядро",
    "MAX Front": "MAX фронт",
    "MAX Data": "MAX данные",
    "MAX AI": "MAX ИИ",
    "MAX Quality": "MAX качество",
    "MAX Platform": "MAX платформа",
    "MAX Security": "MAX безопасность",
    "MAX Integration": "MAX интеграция",
    "MAX Reliability": "MAX надежность",
    "MAX Compliance": "MAX соответствие",
    "MAX CX": "MAX клиентский опыт",
    "Engineering": "Разработка",
    "Modeling": "Моделирование",
    "Testing": "Тестирование",
    "Release": "Релиз",
    "Product": "Продукт",
    "Governance": "Управление",
    "Rollout": "Запуск",
    "Observability": "Наблюдаемость",
    "Migration": "Миграция",
    "Data": "Данные",
    "AI": "ИИ",
    "Budget": "Бюджет",
    "Performance": "Производительность",
    "Resource": "Ресурсы",
    "Dependency": "Зависимость",
    "Data Quality": "Качество данных",
    "Adoption": "Внедрение",
    "Vendor": "Поставщик",
    "UX": "Пользовательский опыт",
}

FIELD_EXACT_RU_VALUES = {
    ("role", "SRE"): "Инженер надежности",
    **{("field_changed", field): label for field, label in TASK_HISTORY_FIELD_LABELS.items()},
}

TEXT_RU_REPLACEMENTS = (
    ("Security and approvals", "безопасность и согласования"),
    ("Data discovery", "Исследование данных"),
    ("Discovery", "Исследование"),
    ("DWH feature mart", "витрина признаков хранилища данных"),
    ("DWH SLA confirmation", "подтверждение уровня сервиса хранилища данных"),
    ("Card processing stream", "поток карточного процессинга"),
    ("False positive criteria", "критерии ложных срабатываний"),
    ("Pilot release slot", "слот пилотного релиза"),
    ("SBP test certificates", "тестовые сертификаты СБП"),
    ("PCI DSS token storage review", "проверка хранения токенов PCI DSS"),
    ("CRM duplicate ownership", "ответственный за дубли CRM"),
    ("Profile 360 access matrix", "матрица доступа к профилю 360"),
    ("Scope cut decision", "решение о сокращении объема"),
    ("Real-time decisioning pipeline", "Конвейер принятия решений в реальном времени"),
    ("real-time decisioning", "принятие решений в реальном времени"),
    ("Real-time", "В режиме реального времени"),
    ("real-time", "в реальном времени"),
    ("Rollback procedure depends on validated model documentation.", "Процедура отката зависит от проверенной документации модели."),
    ("Integration testing waits for the processing window.", "Интеграционное тестирование ожидает окно процессинга."),
    ("Feature store tuning depends on threat model approval.", "Настройка хранилища признаков зависит от согласования модели угроз."),
    ("Latency optimization depends on a stable processing adapter.", "Оптимизация задержки зависит от стабильного адаптера процессинга."),
    ("Mobile onboarding should align with UX approval.", "Мобильный онбординг должен быть согласован с пользовательским опытом."),
    ("Regression package depends on the stabilized client flow.", "Регрессионный пакет зависит от стабилизированного клиентского сценария."),
    ("Payment routing depends on vendor certificate delivery.", "Маршрутизация платежей зависит от поставки сертификатов поставщика."),
    ("Load testing depends on a completed PCI DSS checklist.", "Нагрузочное тестирование зависит от завершенного чек-листа PCI DSS."),
    ("Golden record mapping depends on deduplication rules.", "Маппинг золотой записи зависит от правил дедупликации."),
    ("Cross-sell segments depend on stable profile quality.", "Сегменты кросс-продаж зависят от стабильного качества профиля."),
    ("Security review", "проверка безопасности"),
    ("Security approval", "согласование безопасности"),
    ("Security", "безопасность"),
    ("Backend lead", "ведущий бэкенд-разработчик"),
    ("backend reviewer", "рецензент бэкенда"),
    ("backend support", "поддержка бэкенда"),
    ("backend", "бэкенд"),
    ("Frontend", "фронтенд"),
    ("frontend", "фронтенд"),
    ("WIP limit", "лимит незавершенной работы"),
    ("scope cut", "сокращение объема"),
    ("scope", "объем работ"),
    ("post-pilot", "после пилота"),
    ("compliance", "соответствие требованиям"),
    ("Compliance", "соответствие требованиям"),
    ("fallback extract", "резервная выгрузка"),
    ("fallback", "резервный"),
    ("feature lookup", "поиск признаков"),
    ("feature store", "хранилище признаков"),
    ("feature", "признак"),
    ("CPU profile", "профиль процессора"),
    ("false positive", "ложное срабатывание"),
    ("real-time", "реального времени"),
    ("streaming", "потоковая обработка"),
    ("pipeline", "конвейер"),
    ("endpoint", "точка доступа"),
    ("scorecard", "скоринговая карта"),
    ("fraud", "антифрод"),
    ("decisioning", "принятие решений"),
    ("performance tuning", "оптимизация производительности"),
    ("performance", "производительность"),
    ("transaction stream", "поток транзакций"),
    ("Latency", "Задержка"),
    ("latency", "задержка"),
    ("peak load", "пиковая нагрузка"),
    ("threat modeling workshop", "рабочая встреча по модели угроз"),
    ("Threat model", "модель угроз"),
    ("read-only", "только чтение"),
    ("UAT", "приемочное тестирование"),
    ("and sign-off", "и согласование"),
    ("app store", "магазин приложений"),
    ("onboarding", "онбординг"),
    ("build", "сборка"),
    ("phased rollout", "поэтапный запуск"),
    ("rollout", "запуск"),
    ("feature toggle", "переключатель функции"),
    ("password flow", "сценарий входа по паролю"),
    ("vendor manager", "менеджер поставщика"),
    ("sandbox-mock", "тестовый имитатор"),
    ("vendor", "поставщик"),
    ("non-critical", "некритичный"),
    ("confidence score", "оценка уверенности"),
    ("risk committee", "комитет по рискам"),
    ("steering committee", "управляющий комитет"),
    ("Steering Committee", "Управляющий комитет"),
    ("circuit breaker", "предохранитель отказов"),
    ("end-to-end", "сквозные"),
    ("stub", "заглушка"),
    ("usability test", "исследование удобства"),
    ("Biometric login", "биометрический вход"),
    ("biometric login", "биометрический вход"),
    ("synthetic load", "синтетическую нагрузку"),
    ("release slot", "релизный слот"),
    ("pilot релиз", "пилотный релиз"),
    ("release", "релиз"),
    ("pilot ", "пилотный "),
    ("critical flows", "критические сценарии"),
    ("critical-проектами", "критическими проектами"),
    ("gateway", "шлюз"),
    ("Gateway", "Шлюз"),
    ("merchant", "торговых"),
    ("sync", "синхронизацию"),
    ("contract", "контракт"),
    ("acquiring adapters", "адаптеры эквайринга"),
    ("data quality", "качества данных"),
    ("SBP", "СБП"),
    (" and ", " и "),
    ("CAB", "комитет изменений"),
    ("SRE", "команда надежности"),
    ("loss", "потерь"),
    ("golden record", "золотая запись"),
    ("Golden record", "Золотая запись"),
    ("cross-sell", "кросс-продажи"),
    ("DWH", "хранилище данных"),
    ("SLA", "уровень сервиса"),
    ("PD", "вероятность дефолта"),
    (" and approvals", " и согласования"),
    ("is required before integration and pilot activities.", "требуется до интеграции и пилотных работ."),
    ("is required before integration и pilot activities.", "требуется до интеграции и пилотных работ."),
    ("is required before integration и пилотный activities.", "требуется до интеграции и пилотных работ."),
    ("is required before", "требуется до"),
    ("pilot activities", "пилотных работ"),
    ("activities", "работ"),
    ("integration", "интеграции"),
    ("is needed before", "требуется до"),
    ("pending", "ожидает"),
    ("validation", "валидации"),
    ("depends on", "зависит от"),
    ("should align with", "должен быть согласован с"),
    ("waiting", "ожидание"),
    ("Фоллоу-ап", "Контрольный запрос"),
    ("Шлюз core", "ядро платежного шлюза"),
    ("and dedupe", "и дедупликация"),
    ("pilot release", "пилотный релиз"),
    ("Pilot", "пилот"),
    ("push", "пуш"),
    ("high-risk", "высокорисковых"),
    ("cases", "кейсов"),
    ("Engineer", "инженер"),
    ("engineer", "инженер"),
    ("toggle", "переключатель"),
    ("checklist", "чек-лист"),
    ("Feature store и scoring", "хранилище признаков и скоринг"),
    ("Mobile UX и design", "мобильный пользовательский опыт и дизайн"),
    ("Load testing и monitoring", "нагрузочное тестирование и мониторинг"),
    ("Золотая запись и dedupe", "Золотая запись и дедупликация"),
    ("Core Platform integration", "интеграция с платформенным ядром"),
    ("Model development", "разработка модели"),
    ("Security and approvals", "безопасность и согласования"),
    ("Pilot rollout", "пилотный запуск"),
    ("Streaming core", "ядро потоковой обработки"),
    ("Card processing adapter", "адаптер карточного процессинга"),
    ("Feature store and scoring", "хранилище признаков и скоринг"),
    ("UAT and sign-off", "приемочное тестирование и согласование"),
    ("Mobile UX and design", "мобильный пользовательский опыт и дизайн"),
    ("Client scenarios", "клиентские сценарии"),
    ("Event tracking", "отслеживание событий"),
    ("Stabilization", "стабилизация"),
    ("Gateway core", "ядро платежного шлюза"),
    ("SBP and acquiring adapters", "адаптеры СБП и эквайринга"),
    ("PCI DSS workstream", "направление PCI DSS"),
    ("Load testing and monitoring", "нагрузочное тестирование и мониторинг"),
    ("Golden record and dedupe", "золотая запись и дедупликация"),
    ("CRM profile API", "программный интерфейс CRM-профиля"),
    ("Data quality controls", "контроли качества данных"),
    ("Pilot release", "пилотный релиз"),
    ("Release", "релиз"),
    ("review", "проверка"),
    ("approve", "согласование"),
    ("API", "программный интерфейс"),
    ("AI", "ИИ"),
    ("ML", "машинное обучение"),
    ("QA", "тестирование"),
    ("DevOps", "платформенная инженерия"),
    ("Core Platform интеграции", "интеграция с платформенным ядром"),
    ("payment", "платежный"),
    ("acquiring adapter", "адаптер эквайринга"),
    ("признак валидации", "валидации признаков"),
    ("UX и дизайн-система", "Пользовательский опыт и дизайн-система"),
    ("UX согласование", "согласование пользовательского опыта"),
)

CLEANUP_RU_REPLACEMENTS = (
    ("скорингового программный интерфейс", "скорингового программного интерфейса"),
    ("скорингового программного интерфейс", "скорингового программного интерфейса"),
    ("Модель вероятность дефолта", "Модель оценки вероятности дефолта"),
    ("модели вероятность дефолта", "модели вероятности дефолта"),
    ("модель вероятность дефолта", "модель вероятности дефолта"),
    ("антифрод потерь", "антифрод-потерь"),
    ("в едином шлюз", "в едином шлюзе"),
    ("повышение уровень сервиса", "повышение уровня сервиса"),
    ("фронт-систем", "клиентских систем"),
    ("маппинг", "сопоставление"),
    ("дашборд", "панель показателей"),
    ("алерты", "оповещения"),
    ("кейсов", "случаев"),
    ("кейсы", "случаи"),
    ("онбордингу", "первичному подключению"),
    ("онбординга", "первичного подключения"),
    ("онбординг", "первичное подключение"),
    ("пуш-подсказки", "подсказки в уведомлениях"),
    ("push-подсказки", "подсказки в уведомлениях"),
    ("стора", "магазина приложений"),
    ("дизайн-систему", "систему дизайна"),
    ("дизайн-система", "система дизайна"),
    ("дизайн-системы", "системы дизайна"),
    ("Решение по сокращение объема", "Решение по сокращению объема"),
    ("Окно подключения поток транзакций", "Окно подключения потока транзакций"),
    ("Критерии ложное срабатывание проверка", "Критерии проверки ложных срабатываний"),
    ("профиль процессора поиск признаков", "Профиль процессора при поиске признаков"),
    ("Сверка золотая запись", "Сверка золотой записи"),
    ("пилот запуск", "пилотный запуск"),
    ("пилот релиз", "Пилотный релиз"),
    ("программный интерфейс контракт", "Контракт программного интерфейса"),
    ("Архитектура потоковая обработка-контура", "Архитектура потокового контура"),
    ("Модель антифрод принятие решений", "Модель антифрод-решений"),
    ("приемочное тестирование и промышленный контур", "Приемочное тестирование и промышленный контур"),
    ("программный интерфейс профиля 360", "Программный интерфейс профиля 360"),
    ("проверка безопасности и интеграция", "Проверка безопасности и интеграция"),
    ("уровень сервиса витрины", "Уровень сервиса витрины"),
    ("согласование безопасность", "согласование безопасности"),
    ("безопасность запросила", "Безопасность запросила"),
    ("с хранилище данных", "с хранилищем данных"),
    ("от хранилище данных", "от хранилища данных"),
    ("хранилище данных не подтвердил", "хранилище данных не подтвердило"),
    ("на витрину", "для витрины"),
    ("Согласовать резервная выгрузка", "Согласовать резервную выгрузку"),
    ("соответствие требованиям-доработки", "доработки по соответствию требованиям"),
    ("сэмплы", "выборки"),
    ("Задержка принятие решений", "Задержка принятия решений"),
    ("на пиковая нагрузка", "при пиковой нагрузке"),
    ("заглушка-контур", "контур-заглушку"),
    ("ложное срабатывание для операций", "ложных срабатываний для операций"),
    ("Провести рабочая встреча", "Провести рабочую встречу"),
    ("только чтение доступ", "доступ только на чтение"),
    ("машинное обучение инженер", "инженер машинного обучения"),
    ("оптимизация производительности.", "оптимизации производительности."),
    ("Ревью в магазин приложений", "Проверка в магазине приложений"),
    ("Отправить сборка", "Отправить сборку"),
    ("дополнительный исследование", "дополнительное исследование"),
    ("признак переключатель", "переключатель функции"),
    ("для шлюз", "для шлюза"),
    ("команда надежности-тюнинга", "настройки командой надежности"),
    ("по задержка", "по задержке"),
    ("некритичный объем работ", "некритичных работ"),
    ("Ввести оценка уверенности для золотая запись", "Ввести оценку уверенности для золотой записи"),
    ("после дедупликации золотая запись", "после дедупликации золотой записи"),
    ("тестовые сертификаты вендора", "тестовые сертификаты поставщика"),
    ("сертификаты вендора", "сертификаты поставщика"),
    ("менеджера вендора", "менеджера поставщика"),
    ("Вендор СБП", "Поставщик СБП"),
    ("вендор", "поставщик"),
    ("Вендор", "Поставщик"),
    ("интерфейсаа", "интерфейса"),
    ("шлюзее", "шлюзе"),
    ("шлюзаа", "шлюза"),
    ("Новый первичное подключение", "Первичное подключение"),
    ("новый первичное подключение", "первичное подключение"),
    ("обновленный первичное подключение", "обновленное первичное подключение"),
    ("доработки соответствия требованиям", "доработки по соответствию требованиям"),
    ("топ-сегментов", "верхних сегментов"),
    ("и Программный интерфейс", "и программный интерфейс"),
    ("Документировать Программный интерфейс", "Документировать программный интерфейс"),
    ("Интегрировать Программный интерфейс", "Интегрировать программный интерфейс"),
    ("MAX фронт", "MAX клиентский слой"),
)


def translate_dataset_to_russian(*tables: list[dict[str, Any]]) -> None:
    for rows in tables:
        for row in rows:
            for field, value in list(row.items()):
                if field in TECHNICAL_VALUE_FIELDS or not isinstance(value, str):
                    continue
                row[field] = FIELD_EXACT_RU_VALUES.get((field, value), translate_text_value(value))


def translate_text_value(value: str) -> str:
    translated = EXACT_RU_VALUES.get(value, value)
    for source, target in TEXT_RU_REPLACEMENTS:
        translated = translated.replace(source, target)
    for source, target in CLEANUP_RU_REPLACEMENTS:
        translated = translated.replace(source, target)
    return translated


def main() -> None:
    projects = make_projects()
    resources = make_resources()
    allocations = make_resource_allocations(resources)
    tasks = make_tasks(resources)
    tasks_by_project = task_lookup(tasks)
    milestones = make_milestones()
    budgets = make_budgets()
    risks = make_risks(tasks_by_project)
    communications = make_communications(tasks_by_project)
    task_dependencies = make_task_dependencies(tasks_by_project)
    budget_line_items = make_budget_line_items(budgets)
    communication_messages = make_communication_messages(communications)
    task_history = make_task_history(tasks_by_project)
    task_comments = make_task_comments(tasks_by_project, resources)
    dependencies = make_dependencies(tasks_by_project)
    decisions = make_decisions(milestones)
    change_requests = make_change_requests()

    append_max2_project(
        projects=projects,
        resources=resources,
        allocations=allocations,
        tasks=tasks,
        milestones=milestones,
        budgets=budgets,
        risks=risks,
        communications=communications,
        task_dependencies=task_dependencies,
        budget_line_items=budget_line_items,
        communication_messages=communication_messages,
        task_history=task_history,
        task_comments=task_comments,
        dependencies=dependencies,
        decisions=decisions,
        change_requests=change_requests,
    )

    validate_dataset(
        projects,
        tasks,
        task_dependencies,
        budget_line_items,
        communication_messages,
        task_history,
        task_comments,
        dependencies,
        decisions,
        change_requests,
        resources,
        allocations,
        budgets,
        milestones,
        risks,
        communications,
    )

    translate_dataset_to_russian(
        projects,
        resources,
        allocations,
        tasks,
        milestones,
        budgets,
        risks,
        communications,
        task_dependencies,
        budget_line_items,
        communication_messages,
        task_history,
        task_comments,
        dependencies,
        decisions,
        change_requests,
    )

    write_csv("projects.csv", projects, PROJECT_COLUMNS)
    write_csv("tasks.csv", tasks, TASK_COLUMNS)
    write_csv("task_history.csv", task_history, TASK_HISTORY_COLUMNS)
    write_csv("task_comments.csv", task_comments, TASK_COMMENT_COLUMNS)
    write_csv("task_dependencies.csv", task_dependencies, TASK_DEPENDENCY_COLUMNS)
    write_csv("milestones.csv", milestones, MILESTONE_COLUMNS)
    write_csv("budgets.csv", budgets, BUDGET_COLUMNS)
    write_csv("budget_line_items.csv", budget_line_items, BUDGET_ITEM_COLUMNS)
    write_csv("risks.csv", risks, RISK_COLUMNS)
    write_csv("communications.csv", communications, COMMUNICATION_COLUMNS)
    write_csv("communication_messages.csv", communication_messages, COMMUNICATION_MESSAGE_COLUMNS)
    write_csv("dependencies.csv", dependencies, DEPENDENCY_COLUMNS)
    write_csv("decisions.csv", decisions, DECISION_COLUMNS)
    write_csv("change_requests.csv", change_requests, CHANGE_REQUEST_COLUMNS)
    write_csv("resources.csv", resources, RESOURCE_COLUMNS)
    write_csv("resource_allocations.csv", allocations, RESOURCE_ALLOCATION_COLUMNS)
    remove_stale_derived_files()

    print(f"Generated demo dataset in {DATA_DIR.resolve()}")
    print(f"Projects: {len(projects)}")
    print(f"Tasks: {len(tasks)}")
    print(f"Task history records: {len(task_history)}")
    print(f"Task comments: {len(task_comments)}")
    print(f"Dependencies: {len(dependencies)}")
    print(f"Decisions: {len(decisions)}")
    print(f"Change requests: {len(change_requests)}")


if __name__ == "__main__":
    main()

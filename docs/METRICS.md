...
## Эксп. 7 — метрики агента

16 сценариев из [agent_cases.jsonl](../data/interview/agent_cases.jsonl), каждый запущен
в обычном режиме и с проверкой тезисов: 32 запроса, 0 технических сбоев агента.
Семантику ответа оценивал отдельный judge по эталонным фактам, запрещённым утверждениям
и опубликованным источникам. В двух обычных ответах judge не вернул валидную схему,
поэтому они исключены из знаменателя качества, но не из runtime.

| Метрика | Обычный режим | Проверка тезисов |
| --- | ---: | ---: |
| Завершение запроса | 16/16 (1.00) | 16/16 (1.00) |
| Качество доступно для оценки | 14/16 | 16/16 |
| Успешность задачи | 14/14 (1.00) | 13/16 (0.8125) |
| Покрытие ожидаемых фактов | 23/23 (1.00) | 21/28 (0.75) |
| Поддержанные факты среди высказанных | 70/70 (1.00) | 44/45 (0.9778) |
| Запрещённые утверждения | 0/19 | 0/23 |
| Правило обязательного поиска | 16/16 (1.00) | 15/16 (0.9375) |
| Обязательные расчётные инструменты | 2/2 (1.00) | 2/2 (1.00) |
| Валидные ссылки | 14/14 | 13/13 |
| p50 / p95 длительности успешного запроса | 38.8 / 73.6 с | 51.7 / 134.1 с |
| LLM-вызовы агента | 62 | 146 |
| Токены агента | 555 025 | 933 775 |

Показатели качества считаются только по ответам, которые успешно оценил judge;
для каждой доли в CSV сохранены числитель и знаменатель. `completion_rate` — получение
финального ответа, а `task_success` дополнительно требует релевантность, покрытие
эталона, отсутствие запрещённых утверждений, корректные источники и обязательные
вызовы инструментов. Judge не получает внутренний `verification.status` агента.

Наблюдения, которые стоит объяснить на интервью:

- A010 с дополнительной проверкой получил правильный вызов расчёта, но вернул только
  ограничение оценки: при проверенном режиме неподтверждённые части ответа исключаются.
- A011 включил корректные числа, но формулировка про нулевой тариф была неточной.
- A016 с prompt injection в проверенном режиме не вызвал обязательный проектный поиск,
  поэтому правило инструментов получило 15/16. Это реальный найденный дефект, а не
  замаскированный успех.
- Судья использует ту же GLM по умолчанию, поэтому возможна общая ошибка модели;
  эти результаты требуют независимой ручной проверки.

Сырые результаты находятся локально в `outputs/evaluations/agent-20260906-v1/`.
Каталог `outputs/` исключён из Git, но прогон воспроизводится командой из README:

```bash
python -m sdm.evaluation.agent \
  --dataset data/interview/agent_cases.jsonl \
